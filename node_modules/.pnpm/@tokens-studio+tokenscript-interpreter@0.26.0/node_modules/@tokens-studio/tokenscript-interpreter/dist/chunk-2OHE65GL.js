import { SupportedFormats, ReservedKeyword, UNINTERPRETED_KEYWORDS } from './chunk-LIXBSHBJ.js';
...
${spec}`
  ...
  validate(tokenType, value, visitedUrls = /* @__PURE__ */ new Set(), basePath = [], depth = 0) {
    ...
    console.log("[TokenManager.validate] tokenType:", tokenType, "hasSchema:", !!spec?.schema, "schemaType:", spec?.schema?.type, "hasItems:", !!spec?.schema?.items);
    if (!spec?.schema) {
      console.log("[TokenManager.validate] No schema, returning valid");
      return [{ valid: true, tokenType }];
    }
    if (spec.schema.type === "list" && spec.schema.items) {
      console.log("[TokenManager.validate] Entering list validation branch");
  ...
  validateListItems(value, itemsSpec, visitedUrls, basePath, depth) {
    const results = [];
    console.log("[validateListItems] value type:", value?.constructor?.name, "isListSymbol:", value instanceof ListSymbol);
    if (!(value instanceof ListSymbol)) {
      console.log("[validateListItems] NOT a ListSymbol, returning empty results");
      return results;
    }
    const items = value.value;
    console.log("[validateListItems] items count:", items.length, "itemsSpec:", JSON.stringify(itemsSpec));
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const itemPath = [...basePath, i];
      console.log("[validateListItems] Processing item", i, "type:", item?.constructor?.name);
      if (itemsSpec.type === "object" && itemsSpec.properties) {
        const itemMap = this.getValueMap(item);
        console.log("[validateListItems] itemMap:", itemMap ? `Map(${itemMap.size})` : null, "keys:", itemMap ? Array.from(itemMap.keys()) : []);
        if (itemMap) {
          const propResults = this.validateObjectProperties(
            itemMap,
          ...
          console.log("[validateListItems] propResults:", JSON.stringify(propResults));
          results.push(...propResults);
        }
      }
  ...
  validateObjectProperties(valueMap, properties, visitedUrls, basePath, depth) {
    const results = [];
    console.log("[validateObjectProperties] properties:", Object.keys(properties));
    for (const [propName, propSpec] of Object.entries(properties)) {
      const typedPropSpec = propSpec;
      console.log("[validateObjectProperties] checking prop:", propName, "type:", typedPropSpec.type, "url:", typedPropSpec.url);
      if (typedPropSpec.type !== "token" || !typedPropSpec.url) {
        console.log("[validateObjectProperties] skipping - not a token type");
        continue;
      }
      ...
      console.log("[validateObjectProperties] propValue for", propName, ":", propValue ? propValue.constructor.name : null);
      if (!propValue) {
        console.log("[validateObjectProperties] skipping - no value for prop");
        continue;
      }
      const referencedSpec = this.getSpec(typedPropSpec.url);
      console.log("[validateObjectProperties] referencedSpec for", typedPropSpec.url, ":", referencedSpec ? referencedSpec.name : null);
      if (!referencedSpec) {
        console.log("[validateObjectProperties] skipping - spec not found for URL:", typedPropSpec.url);
        continue;
      }
      const newVisited = new Set(visitedUrls);
...
//# sourceMappingURL=chunk-2OHE65GL.js.map
//# sourceMappingURL=chunk-2OHE65GL.js.map