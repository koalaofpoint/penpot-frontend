import { DictionarySymbol, serializeInterpreterResult, stringifyInterpreterResult, isObject, isSome, LexerError, isSpace, isNumber2, isAlpha, isAlphaNumeric, ParserError, isLanguageError, NullSymbol, isTokenscriptSymbol, StringSymbol, NumberWithUnitSymbol, NumberSymbol, InterpreterError, BooleanSymbol, isNull, jsValueToSymbolType, ListSymbol, parseHexWithAlpha, ColorSymbol, typeName, basicSymbolTypes, typeEquals, ProcessorError, TokenSymbol, isArray, isString, getKeyAlt, isNumber, isBoolean, safeParseInt, serializeError, ensureValidAlpha, formatHex8, isTransparent, getResultTypeName, LanguageError } from './chunk-TUFIHLPU.js';
import { __name, SupportedFormats, ReservedKeyword, __export, __publicField, UNINTERPRETED_KEYWORDS } from './chunk-SLVAZICD.js';
...
__name(_UnaryOpNode, "UnaryOpNode");
...
__name(_ImplicitListNode, "ImplicitListNode");
...
__name(_FunctionCallNode, "FunctionCallNode");
...
__name(_IdentifierNode, "IdentifierNode");
...
__name(_HexColorNode, "HexColorNode");
...
__name(_BooleanNode, "BooleanNode");
...
__name(_ElementWithUnitNode, "ElementWithUnitNode");
...
__name(_TypeDeclNode, "TypeDeclNode");
var TypeDeclNode = _TypeDeclNode;
var attributesToString = /* @__PURE__ */ __name((attrs) => attrs.join("."), "attributesToString");
var identifiersChainToString = /* @__PURE__ */ __name((attrs) => attributesToString(attrs.map((x) => x.name)), "identifiersChainToString");
...
__name(_ReassignNode, "ReassignNode");
...
__name(_IfConditionNode, "IfConditionNode");
...
__name(_StatementListNode, "StatementListNode");
...
__name(_AttributeAccessNode, "AttributeAccessNode");
...
__name(collectReferenceNodes, "collectReferenceNodes");
...
__name(renameReferences, "renameReferences");
...
__name(_DependencyTracker, "DependencyTracker");
...
__name(getAffectedTokens, "getAffectedTokens");
...
__name(getBrokenReferences, "getBrokenReferences");
function getRenamedReferences(_result, modifiedTokens) {
  return modifiedTokens;
}
__name(getRenamedReferences, "getRenamedReferences");
...
__name(getModifiedDependants, "getModifiedDependants");
...
__name(getTokenError, "getTokenError");
...
__name(_PrefixExtractor, "PrefixExtractor");
...
var _PrefixManager = class _PrefixManager {
  ...
  toSymbol(value2) {
    ...
    if (value2 === null) return new NullSymbol(this.config);
    if (isTokenscriptSymbol(value2)) return value2;
    return new StringSymbol(String(value2), this.config);
  }
};
__name(_PrefixManager, "PrefixManager");
...
__name(_ReadinessTracker, "ReadinessTracker");
...
__name(_ResolutionNotifier, "ResolutionNotifier");
...
function recomposeUnit(value2, units, _config) {
  ...
  return new NumberWithUnitSymbol(value2, validUnits[0]);
}
__name(recomposeUnit, "recomposeUnit");
...
__name(mathWrapper, "mathWrapper");
function equalityComparisonWrapper(fn2) {
  return (a, b) => {
    ...
    return new BooleanSymbol(fn2(a.value, b.value));
  };
}
__name(equalityComparisonWrapper, "equalityComparisonWrapper");
...
__name(orderingComparisonWrapper, "orderingComparisonWrapper");
...
__name(powerWrapper, "powerWrapper");
...
__name(multiplyDivideWrapper, "multiplyDivideWrapper");
...
__name(_ReturnSignal, "ReturnSignal");
...
var _Interpreter = class _Interpreter {
  ...
  visitNumNode(node2) {
    return new NumberSymbol(node2.value, this.config);
  }
  visitStringNode(node2) {
    return new StringSymbol(node2.value, this.config);
  }
  /**
  ...
  visitIdentifierNode(node2) {
    return this.symbolTable.get(node2.name) || this.getReference(node2.name) || new StringSymbol(node2.name, this.config);
  }
  ...
  visitHexColorNode(node2) {
    const { color, alpha } = parseHexWithAlpha(node2.value);
    return new ColorSymbol(color, "Hex", alpha, this.config);
  }
  visitBooleanNode(node2) {
    return new BooleanSymbol(node2.value, this.config);
  }
  visitNullNode(_node) {
  ...
  visitElementWithUnitNode(node2) {
    ...
    return new NumberWithUnitSymbol(valNodeVisit?.value, node2.unit, this.config);
  }
  ...
  visitReassignNode(node2) {
    ...
    if (node2.isAttributeAssignment()) {
      ...
      if (typeEquals(value2.type, "list")) ; else if (!typeEquals(existingVar.type, value2.type)) {
...
__name(_Interpreter, "Interpreter");
...
__name(extractDependencyInfo, "extractDependencyInfo");
...
var _TokenInterpreter = class _TokenInterpreter {
  ...
  flattenDictionaryToCache(tokenName, value2) {
    if (!(value2 instanceof DictionarySymbol) || !value2.value) {
      return [];
    }
...
__name(_TokenInterpreter, "TokenInterpreter");
...
function parseValueToSymbol(value2, config, parsers = defaultObjectParsers) {
  ...
  return jsValueToSymbolType(value2, config);
}
__name(parseValueToSymbol, "parseValueToSymbol");
function createTokenSymbol(value2, tokenType = "unknown", config, parsers = defaultObjectParsers, metadata) {
  ...
  return new TokenSymbol(tokenType, value2, config, metadata);
}
__name(createTokenSymbol, "createTokenSymbol");
...
__name(buildValueFromResolvedFields, "buildValueFromResolvedFields");
...
__name(createTokenSymbolFromResolvedFields, "createTokenSymbolFromResolvedFields");
...
var _DependencyGraph = class _DependencyGraph {
  ...
  findCycle(candidateNodes) {
    ...
    const path = [];
    const dfs = /* @__PURE__ */ __name((node2) => {
      visited.add(node2);
...
__name(matchesObjectParser, "matchesObjectParser");
function extractStringFields(value2, parentPath, objectParsers = defaultObjectParsers) {
  const result = /* @__PURE__ */ new Map();
  const processValue = /* @__PURE__ */ __name((val, path) => {
    if (isString(val)) {
      result.set(path, val);
...
__name(extractStringFields, "extractStringFields");
...
function setTokenValue(value2) {
  return isString(value2) ? { $value: value2 } : value2;
}
__name(setTokenValue, "setTokenValue");
...
__name(flattenTokensObject, "flattenTokensObject");
function normalizeToTokenData(value2) {
  if (typeof value2 === "string") {
    return { $value: value2 };
  }
  if (isObject(value2) && "$value" in value2) {
    const obj = value2;
...
__name(normalizeToTokenData, "normalizeToTokenData");
...
__name(recordToMap, "recordToMap");
function isNested(obj) {
  return Object.keys(obj).some((key) => {
    const value2 = obj[key];
    return isObject(value2) && !key.startsWith("$");
  });
}
__name(isNested, "isNested");
...
__name(errorToValidationIssue, "errorToValidationIssue");
...
__name(getTokenDependencyGraph, "getTokenDependencyGraph");
var _PrefixResolver = class _PrefixResolver {
  ...
  validateTokenType(tokenName, tokenType, value2) {
    ...
    let valueToValidate;
    if (value2 instanceof TokenSymbol) {
...
__name(_PrefixResolver, "PrefixResolver");
...
var _TokenResolver = class _TokenResolver {
  ...
  build(tokens, config, objectParsers) {
    ...
    const callbacks = {
      onResolve: /* @__PURE__ */ __name((tokenName, value2) => {
        output.set(tokenName, value2);
      }, "onResolve"),
      onError: /* @__PURE__ */ __name((tokenName, _error, originalValue) => {
  ...
  createOutputCallbacks() {
    const output = /* @__PURE__ */ new Map();
    const callbacks = {
      onResolve: /* @__PURE__ */ __name((name, value2) => {
        output.set(name, value2);
      }, "onResolve"),
      onError: /* @__PURE__ */ __name((_name, _error) => {
      }, "onError")
    };
...
__name(_TokenResolver, "TokenResolver");
...
__name(buildTokens, "buildTokens");
...
__name(flattenChildrenObject, "flattenChildrenObject");
function flattenChildrenMap(map, prefix, callback) {
  for (const [key, value2] of map.entries()) {
    ...
    if (value2 instanceof DictionarySymbol) {
...
__name(flattenChildrenMap, "flattenChildrenMap");
...
var _NestedObjectBuilder = class _NestedObjectBuilder {
  ...
  onResolve(tokenName, value2) {
    const serialized = serializeInterpreterResult(value2, { stringify: true });
...
__name(_NestedObjectBuilder, "NestedObjectBuilder");
...
__name(_FlatObjectBuilder, "FlatObjectBuilder");
...
var _StringMapBuilder = class _StringMapBuilder extends MapBuilder {
  onResolve(tokenName, value2) {
    ...
    this.result.set(tokenName, stringifyInterpreterResult(value2));
  }
  getResult() {
    const stringMap = /* @__PURE__ */ new Map();
    for (const [key, value2] of this.result.entries()) {
      stringMap.set(key, typeof value2 === "string" ? value2 : stringifyInterpreterResult(value2));
    }
    return stringMap;
  }
};
__name(_StringMapBuilder, "StringMapBuilder");
var StringMapBuilder = _StringMapBuilder;

// node_modules/@ark/util/out/arrays.js
var liftArray = /* @__PURE__ */ __name((data) => Array.isArray(data) ? data : [data], "liftArray");
var spliterate = /* @__PURE__ */ __name((arr, predicate) => {
  const result = [[], []];
...
var includes = /* @__PURE__ */ __name((array, element) => array.includes(element), "includes");
var range = /* @__PURE__ */ __name((length, offset = 0) => [...new Array(length)].map((_, i) => i + offset), "range");
var append = /* @__PURE__ */ __name((to, value2, opts) => {
  if (to === void 0) {
...
var conflatenate = /* @__PURE__ */ __name((to, elementOrList) => {
...
var conflatenateAll = /* @__PURE__ */ __name((...elementsOrLists) => elementsOrLists.reduce(conflatenate, []), "conflatenateAll");
var appendUnique = /* @__PURE__ */ __name((to, value2, opts) => {
  if (to === void 0)
...
var groupBy = /* @__PURE__ */ __name((array, discriminant) => array.reduce((result, item) => {
  const key = item[discriminant];
  result[key] = append(result[key], item);
  return result;
}, {}), "groupBy");
var arrayEquals = /* @__PURE__ */ __name((l, r, opts) => l.length === r.length && l.every(opts?.isEqual ? (lItem, i) => opts.isEqual(lItem, r[i]) : (lItem, i) => lItem === r[i]), "arrayEquals");

// node_modules/@ark/util/out/domain.js
var hasDomain = /* @__PURE__ */ __name((data, kind) => domainOf(data) === kind, "hasDomain");
var domainOf = /* @__PURE__ */ __name((data) => {
...
__name(_InternalArktypeError, "InternalArktypeError");
var InternalArktypeError = _InternalArktypeError;
var throwInternalError = /* @__PURE__ */ __name((message) => throwError(message, InternalArktypeError), "throwInternalError");
var throwError = /* @__PURE__ */ __name((message, ctor = Error) => {
...
var throwParseError = /* @__PURE__ */ __name((message) => throwError(message, ParseError), "throwParseError");
var noSuggest = /* @__PURE__ */ __name((s) => ` ${s}`, "noSuggest");
var ZeroWidthSpace = "\u200B";

// node_modules/@ark/util/out/flatMorph.js
var flatMorph = /* @__PURE__ */ __name((o, flatMapEntry) => {
  const result = {};
...
var isKeyOf = /* @__PURE__ */ __name((k, o) => k in o, "isKeyOf");
var hasKey = /* @__PURE__ */ __name((o, k) => k in o, "hasKey");
var _DynamicBase = class _DynamicBase {
  constructor(properties) {
    Object.assign(this, properties);
  }
};
__name(_DynamicBase, "DynamicBase");
var DynamicBase = _DynamicBase;
var _a;
var NoopBase = (_a = class {
}, __name(_a, "NoopBase"), _a);
var _CastableBase = class _CastableBase extends NoopBase {
};
__name(_CastableBase, "CastableBase");
var CastableBase = _CastableBase;
var splitByKeys = /* @__PURE__ */ __name((o, leftKeys) => {
  const l = {};
  const r = {};
  let k;
  for (k in o) {
...
var omit = /* @__PURE__ */ __name((o, keys) => splitByKeys(o, keys)[1], "omit");
var isEmptyObject = /* @__PURE__ */ __name((o) => Object.keys(o).length === 0, "isEmptyObject");
var stringAndSymbolicEntriesOf = /* @__PURE__ */ __name((o) => [
  ...Object.entries(o),
  ...Object.getOwnPropertySymbols(o).map((k) => [k, o[k]])
], "stringAndSymbolicEntriesOf");
var defineProperties = /* @__PURE__ */ __name((base, merged) => (
...
var withAlphabetizedKeys = /* @__PURE__ */ __name((o) => {
...
var enumValues = /* @__PURE__ */ __name((tsEnum) => Object.values(tsEnum).filter((v) => {
  if (typeof v === "number")
    return true;
...
var objectKindOf = /* @__PURE__ */ __name((data) => {
...
var objectKindOrDomainOf = /* @__PURE__ */ __name((data) => typeof data === "object" && data !== null ? objectKindOf(data) ?? "object" : domainOf(data), "objectKindOrDomainOf");
...
var getBuiltinNameOfConstructor = /* @__PURE__ */ __name((ctor) => {
...
var constructorExtends = /* @__PURE__ */ __name((ctor, base) => {
...
var deepClone = /* @__PURE__ */ __name((input) => _clone(input, /* @__PURE__ */ new Map()), "deepClone");
var _clone = /* @__PURE__ */ __name((input, seen) => {
...
var cached = /* @__PURE__ */ __name((thunk) => {
  let result = unset;
  return () => result === unset ? result = thunk() : result;
}, "cached");
var isThunk = /* @__PURE__ */ __name((value2) => typeof value2 === "function" && value2.length === 0, "isThunk");
var _a2;
...
}, __name(_a2, "DynamicFunction"), _a2);
...
var capitalize = /* @__PURE__ */ __name((s) => s[0].toUpperCase() + s.slice(1), "capitalize");
var anchoredRegex = /* @__PURE__ */ __name((regex3) => new RegExp(anchoredSource(regex3), typeof regex3 === "string" ? "" : regex3.flags), "anchoredRegex");
var anchoredSource = /* @__PURE__ */ __name((regex3) => {
  const source = typeof regex3 === "string" ? regex3 : regex3.source;
  return `^(?:${source})$`;
}, "anchoredSource");
var RegexPatterns = {
  negativeLookahead: /* @__PURE__ */ __name((pattern) => `(?!${pattern})`, "negativeLookahead"),
  nonCapturingGroup: /* @__PURE__ */ __name((pattern) => `(?:${pattern})`, "nonCapturingGroup")
};
var Backslash = "\\";
var whitespaceChars = {
  " ": 1,
...
var isNumberLike = /* @__PURE__ */ __name((s) => s.length !== 0 && numberLikeMatcher.test(s), "isNumberLike");
...
var writeMalformedNumericLiteralMessage = /* @__PURE__ */ __name((def, kind) => `'${def}' was parsed as ${numericLiteralDescriptions[kind]} but could not be narrowed to a literal value. Avoid unnecessary leading or trailing zeros and other abnormal notation`, "writeMalformedNumericLiteralMessage");
var isWellFormed = /* @__PURE__ */ __name((def, kind) => kind === "number" ? isWellFormedNumber(def) : isWellFormedInteger(def), "isWellFormed");
var parseKind = /* @__PURE__ */ __name((def, kind) => kind === "number" ? Number(def) : Number.parseInt(def), "parseKind");
var isKindLike = /* @__PURE__ */ __name((def, kind) => kind === "number" ? isNumberLike(def) : isIntegerLike(def), "isKindLike");
var tryParseNumber = /* @__PURE__ */ __name((token, options) => parseNumeric(token, "number", options), "tryParseNumber");
var tryParseWellFormedNumber = /* @__PURE__ */ __name((token, options) => parseNumeric(token, "number", { ...options, strict: true }), "tryParseWellFormedNumber");
var tryParseInteger = /* @__PURE__ */ __name((token, options) => parseNumeric(token, "integer", options), "tryParseInteger");
var parseNumeric = /* @__PURE__ */ __name((token, kind, options) => {
...
var tryParseWellFormedBigint = /* @__PURE__ */ __name((def) => {
...
var register = /* @__PURE__ */ __name((value2) => {
...
var isDotAccessible = /* @__PURE__ */ __name((keyName) => /^[$A-Z_a-z][\w$]*$/.test(keyName), "isDotAccessible");
var baseNameFor = /* @__PURE__ */ __name((value2) => {
...
var serializePrimitive = /* @__PURE__ */ __name((value2) => typeof value2 === "string" ? JSON.stringify(value2) : typeof value2 === "bigint" ? `${value2}n` : `${value2}`, "serializePrimitive");

// node_modules/@ark/util/out/serialize.js
var snapshot = /* @__PURE__ */ __name((data, opts = {}) => _serialize(data, {
  onUndefined: `$ark.undefined`,
  onBigInt: /* @__PURE__ */ __name((n) => `$ark.bigint-${n}`, "onBigInt"),
  ...opts
}, []), "snapshot");
var printable = /* @__PURE__ */ __name((data, opts) => {
...
var stringifyUnquoted = /* @__PURE__ */ __name((value2, indent2, currentIndent) => {
...
var printableOpts = {
  onCycle: /* @__PURE__ */ __name(() => "(cycle)", "onCycle"),
  onSymbol: /* @__PURE__ */ __name((v) => `Symbol(${register(v)})`, "onSymbol"),
  onFunction: /* @__PURE__ */ __name((v) => `Function(${register(v)})`, "onFunction")
};
var _serialize = /* @__PURE__ */ __name((data, opts, seen) => {
  switch (domainOf(data)) {
    case "object": {
      const o = data;
...
var describeCollapsibleDate = /* @__PURE__ */ __name((date) => {
...
var pad = /* @__PURE__ */ __name((value2, length) => String(value2).padStart(length, "0"), "pad");

// node_modules/@ark/util/out/path.js
var appendStringifiedKey = /* @__PURE__ */ __name((path, prop, ...[opts]) => {
...
var stringifyPath = /* @__PURE__ */ __name((path, ...opts) => path.reduce((s, k) => appendStringifiedKey(s, k, ...opts), ""), "stringifyPath");
...
__name(_ReadonlyPath, "ReadonlyPath");
...
var writeUnmatchedGroupCloseMessage = /* @__PURE__ */ __name((char, unscanned) => `Unmatched ${char}${unscanned === "" ? "" : ` before ${unscanned}`}`, "writeUnmatchedGroupCloseMessage");
var writeUnclosedGroupMessage = /* @__PURE__ */ __name((missingChar) => `Missing ${missingChar}`, "writeUnclosedGroupMessage");
...
var $ark = registry;
var reference = /* @__PURE__ */ __name((name) => `${registryName}.${name}`, "reference");
var registeredReference = /* @__PURE__ */ __name((value2) => reference(register(value2)), "registeredReference");
...
__name(_CompiledFunction, "CompiledFunction");
var CompiledFunction = _CompiledFunction;
var compileSerializedValue = /* @__PURE__ */ __name((value2) => hasDomain(value2, "object") || typeof value2 === "symbol" ? registeredReference(value2) : serializePrimitive(value2), "compileSerializedValue");
var compileLiteralPropAccess = /* @__PURE__ */ __name((key, optional = false) => {
  if (typeof key === "string" && isDotAccessible(key))
    return `${optional ? "?" : ""}.${key}`;
  return indexPropAccess(serializeLiteralKey(key), optional);
}, "compileLiteralPropAccess");
var serializeLiteralKey = /* @__PURE__ */ __name((key) => typeof key === "symbol" ? registeredReference(key) : JSON.stringify(key), "serializeLiteralKey");
var indexPropAccess = /* @__PURE__ */ __name((key, optional = false) => `${optional ? "?." : ""}[${key}]`, "indexPropAccess");
...
__name(_NodeCompiler, "NodeCompiler");
var NodeCompiler = _NodeCompiler;

// node_modules/@ark/schema/out/shared/utils.js
var makeRootAndArrayPropertiesMutable = /* @__PURE__ */ __name((o) => (
...
var hasArkKind = /* @__PURE__ */ __name((value2, kind) => value2?.[arkKind] === kind, "hasArkKind");
var isNode = /* @__PURE__ */ __name((value2) => hasArkKind(value2, "root") || hasArkKind(value2, "constraint"), "isNode");
...
var isNodeKind = /* @__PURE__ */ __name((value2) => typeof value2 === "string" && value2 in precedenceByKind, "isNodeKind");
var precedenceOfKind = /* @__PURE__ */ __name((kind) => precedenceByKind[kind], "precedenceOfKind");
var schemaKindsRightOf = /* @__PURE__ */ __name((kind) => rootKinds.slice(precedenceOfKind(kind) + 1), "schemaKindsRightOf");
[
...
var defaultValueSerializer = /* @__PURE__ */ __name((v) => {
...
var compileObjectLiteral = /* @__PURE__ */ __name((ctx) => {
  let result = "{ ";
  for (const [k, v] of Object.entries(ctx))
    result += `${k}: ${compileSerializedValue(v)}, `;
  return result + " }";
}, "compileObjectLiteral");
var implementNode = /* @__PURE__ */ __name((_) => {
...
__name(_ToJsonSchemaError, "ToJsonSchemaError");
var ToJsonSchemaError = _ToJsonSchemaError;
var defaultConfig = {
  dialect: "https://json-schema.org/draft/2020-12/schema",
  useRefs: false,
  fallback: {
    arrayObject: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("arrayObject", ctx), "arrayObject"),
    arrayPostfix: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("arrayPostfix", ctx), "arrayPostfix"),
    defaultValue: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("defaultValue", ctx), "defaultValue"),
    domain: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("domain", ctx), "domain"),
    morph: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("morph", ctx), "morph"),
    patternIntersection: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("patternIntersection", ctx), "patternIntersection"),
    predicate: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("predicate", ctx), "predicate"),
    proto: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("proto", ctx), "proto"),
    symbolKey: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("symbolKey", ctx), "symbolKey"),
    unit: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("unit", ctx), "unit"),
    date: /* @__PURE__ */ __name((ctx) => ToJsonSchema.throw("date", ctx), "date")
  }
};
var ToJsonSchema = {
  Error: ToJsonSchemaError,
  throw: /* @__PURE__ */ __name((...args2) => {
    throw new ToJsonSchema.Error(...args2);
  }, "throw"),
  throwInternalOperandError: /* @__PURE__ */ __name((kind, schema2) => throwInternalError(`Unexpected JSON Schema input for ${kind}: ${printable(schema2)}`), "throwInternalOperandError"),
  defaultConfig
};

// node_modules/@ark/schema/out/config.js
$ark.config ??= {};
var mergeConfigs = /* @__PURE__ */ __name((base, merged) => {
  if (!merged)
    return base;
  const result = { ...base };
  let k;
...
var mergeToJsonSchemaConfigs = /* @__PURE__ */ __name((baseConfig, mergedConfig) => {
  if (!baseConfig)
    return mergedConfig ?? {};
  if (!mergedConfig)
    return baseConfig;
...
var mergeFallbacks = /* @__PURE__ */ __name((base, merged) => {
...
var normalizeFallback = /* @__PURE__ */ __name((fallback) => typeof fallback === "function" ? { default: fallback } : fallback ?? {}, "normalizeFallback");
...
__name(_TraversalError, "TraversalError");
var TraversalError = _TraversalError;
var indent = /* @__PURE__ */ __name((error) => error.toString().split("\n").join("\n  "), "indent");
...
var traverseKey = /* @__PURE__ */ __name((key, fn2, ctx) => {
  if (!ctx)
    return fn2();
  ctx.path.push(key);
  const result = fn2();
...
var _BaseNode = class _BaseNode extends Callable {
  constructor(attachments, $) {
    ...
    __publicField(this, "assert", /* @__PURE__ */ __name((data, pipedFromCtx) => this(data, pipedFromCtx, (errors) => errors.throw()), "assert"));
...
var NodeSelector = {
  applyBoundary: {
    self: /* @__PURE__ */ __name((node2) => [node2], "self"),
    child: /* @__PURE__ */ __name((node2) => [...node2.children], "child"),
    shallow: /* @__PURE__ */ __name((node2) => [...node2.shallowReferences], "shallow"),
    references: /* @__PURE__ */ __name((node2) => [...node2.references], "references")
  },
  applyMethod: {
    filter: /* @__PURE__ */ __name((nodes) => nodes, "filter"),
    assertFilter: /* @__PURE__ */ __name((nodes, from, selector) => {
      if (nodes.length === 0)
        throwError(writeSelectAssertionMessage(from, selector));
      return nodes;
    }, "assertFilter"),
    find: /* @__PURE__ */ __name((nodes) => nodes[0], "find"),
    assertFind: /* @__PURE__ */ __name((nodes, from, selector) => {
      if (nodes.length === 0)
        throwError(writeSelectAssertionMessage(from, selector));
      return nodes[0];
    }, "assertFind")
  },
  normalize: /* @__PURE__ */ __name((selector) => typeof selector === "function" ? { boundary: "references", method: "filter", where: selector } : typeof selector === "string" ? isKeyOf(selector, NodeSelector.applyBoundary) ? { method: "filter", boundary: selector } : { boundary: "references", method: "filter", kind: selector } : { boundary: "references", method: "filter", ...selector }, "normalize")
};
var writeSelectAssertionMessage = /* @__PURE__ */ __name((from, selector) => `${from} had no references matching ${printable(selector)}.`, "writeSelectAssertionMessage");
var typePathToPropString = /* @__PURE__ */ __name((path) => stringifyPath(path, {
  stringifyNonKey: /* @__PURE__ */ __name((node2) => node2.expression, "stringifyNonKey")
}), "typePathToPropString");
var referenceMatcher = /"(\$ark\.[^"]+)"/g;
var compileMeta = /* @__PURE__ */ __name((metaJson) => JSON.stringify(metaJson).replaceAll(referenceMatcher, "$1"), "compileMeta");
var flatRef = /* @__PURE__ */ __name((path, node2) => ({
  path,
  node: node2,
  propString: typePathToPropString(path)
}), "flatRef");
var flatRefsAreEqual = /* @__PURE__ */ __name((l, r) => l.propString === r.propString && l.node.equals(r.node), "flatRefsAreEqual");
var appendUniqueFlatRefs = /* @__PURE__ */ __name((existing, refs) => appendUnique(existing, refs, {
  isEqual: flatRefsAreEqual
}), "appendUniqueFlatRefs");
var appendUniqueNodes = /* @__PURE__ */ __name((existing, refs) => appendUnique(existing, refs, {
  isEqual: /* @__PURE__ */ __name((l, r) => l.equals(r), "isEqual")
}), "appendUniqueNodes");
...
__name(_Disjoint, "Disjoint");
var Disjoint = _Disjoint;
var describeReasons = /* @__PURE__ */ __name((l, r) => `${describeReason(l)} and ${describeReason(r)}`, "describeReasons");
var describeReason = /* @__PURE__ */ __name((value2) => isNode(value2) ? value2.expression : isArray2(value2) ? value2.map(describeReason).join(" | ") || "never" : String(value2), "describeReason");
var writeUnsatisfiableExpressionError = /* @__PURE__ */ __name((expression) => `${expression} results in an unsatisfiable type`, "writeUnsatisfiableExpressionError");

// node_modules/@ark/schema/out/shared/intersections.js
var intersectionCache = {};
var intersectNodesRoot = /* @__PURE__ */ __name((l, r, $) => intersectOrPipeNodes(l, r, {
  $,
  invert: false,
  pipe: false
}), "intersectNodesRoot");
var pipeNodesRoot = /* @__PURE__ */ __name((l, r, $) => intersectOrPipeNodes(l, r, {
  $,
  invert: false,
  pipe: true
}), "pipeNodesRoot");
var intersectOrPipeNodes = /* @__PURE__ */ __name((l, r, ctx) => {
...
var _intersectNodes = /* @__PURE__ */ __name((l, r, ctx) => {
...
var _pipeNodes = /* @__PURE__ */ __name((l, r, ctx) => l.includesTransform || r.includesTransform ? ctx.invert ? pipeMorphed(r, l, ctx) : pipeMorphed(l, r, ctx) : _intersectNodes(l, r, ctx), "_pipeNodes");
var pipeMorphed = /* @__PURE__ */ __name((from, to, ctx) => from.distribute((fromBranch) => _pipeMorphed(fromBranch, to, ctx), (results) => {
...
var _pipeMorphed = /* @__PURE__ */ __name((from, to, ctx) => {
...
__name(_BaseConstraint, "BaseConstraint");
...
var _InternalPrimitiveConstraint = class _InternalPrimitiveConstraint extends BaseConstraint {
  constructor() {
    super(...arguments);
    __publicField(this, "traverseApply", /* @__PURE__ */ __name((data, ctx) => {
...
__name(_InternalPrimitiveConstraint, "InternalPrimitiveConstraint");
var InternalPrimitiveConstraint = _InternalPrimitiveConstraint;
var constraintKeyParser = /* @__PURE__ */ __name((kind) => (schema2, ctx) => {
  if (isArray2(schema2)) {
    if (schema2.length === 0) {
      return;
    }
...
var intersectConstraints = /* @__PURE__ */ __name((s) => {
...
var flattenConstraints = /* @__PURE__ */ __name((inner) => {
...
var unflattenConstraints = /* @__PURE__ */ __name((constraints) => {
  const inner = {};
...
var throwInvalidOperandError = /* @__PURE__ */ __name((...args2) => throwParseError(writeInvalidOperandMessage(...args2)), "throwInvalidOperandError");
var writeInvalidOperandMessage = /* @__PURE__ */ __name((kind, expected, actual) => {
...
var parseGeneric = /* @__PURE__ */ __name((paramDefs, bodyDef, $) => new GenericRoot(paramDefs, bodyDef, $, $, null), "parseGeneric");
var _LazyGenericBody = class _LazyGenericBody extends Callable {
};
__name(_LazyGenericBody, "LazyGenericBody");
...
__name(_GenericRoot, "GenericRoot");
var GenericRoot = _GenericRoot;
var writeUnsatisfiedParameterConstraintMessage = /* @__PURE__ */ __name((name, constraint, arg) => `${name} must be assignable to ${constraint} (was ${arg})`, "writeUnsatisfiedParameterConstraintMessage");
...
var implementation = implementNode({
  ...
  normalize: /* @__PURE__ */ __name((schema2) => typeof schema2 === "function" ? { predicate: schema2 } : schema2, "normalize"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `valid according to ${node2.predicate.name || "an anonymous predicate"}`, "description")
  },
  ...
  intersections: {
    ...
    predicate: /* @__PURE__ */ __name(() => null, "predicate")
  }
});
...
__name(_PredicateNode, "PredicateNode");
...
var implementation2 = implementNode({
  ...
  keys: {
    rule: {
      parse: /* @__PURE__ */ __name((divisor) => Number.isInteger(divisor) ? divisor : throwParseError(writeNonIntegerDivisorMessage(divisor)), "parse")
    }
  },
  normalize: /* @__PURE__ */ __name((schema2) => typeof schema2 === "number" ? { rule: schema2 } : schema2, "normalize"),
  hasAssociatedError: true,
  defaults: {
    description: /* @__PURE__ */ __name((node2) => node2.rule === 1 ? "an integer" : node2.rule === 2 ? "even" : `a multiple of ${node2.rule}`, "description")
  },
  intersections: {
    divisor: /* @__PURE__ */ __name((l, r, ctx) => ctx.$.node("divisor", {
...
var _DivisorNode = class _DivisorNode extends InternalPrimitiveConstraint {
  constructor() {
    super(...arguments);
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data) => data % this.rule === 0, "traverseAllows"));
...
__name(_DivisorNode, "DivisorNode");
var DivisorNode = _DivisorNode;
var Divisor = {
  implementation: implementation2,
  Node: DivisorNode
};
var writeNonIntegerDivisorMessage = /* @__PURE__ */ __name((divisor) => `divisor must be an integer (was ${divisor})`, "writeNonIntegerDivisorMessage");
var greatestCommonDivisor = /* @__PURE__ */ __name((l, r) => {
  let previous;
...
var parseExclusiveKey = {
  ...
  parse: /* @__PURE__ */ __name((flag) => flag || void 0, "parse")
};
var createLengthSchemaNormalizer = /* @__PURE__ */ __name((kind) => (schema2) => {
...
var createDateSchemaNormalizer = /* @__PURE__ */ __name((kind) => (schema2) => {
...
var parseDateLimit = /* @__PURE__ */ __name((limit) => typeof limit === "string" || typeof limit === "number" ? new Date(limit) : limit, "parseDateLimit");
var writeInvalidLengthBoundMessage = /* @__PURE__ */ __name((kind, limit) => `${kind} bound must be a positive integer (was ${limit})`, "writeInvalidLengthBoundMessage");
var createLengthRuleParser = /* @__PURE__ */ __name((kind) => (limit) => {
...
var compileComparator = /* @__PURE__ */ __name((kind, exclusive) => `${isKeyOf(kind, boundKindPairsByLower) ? ">" : "<"}${exclusive ? "" : "="}`, "compileComparator");
var dateLimitToString = /* @__PURE__ */ __name((limit) => typeof limit === "string" ? limit : new Date(limit).toLocaleString(), "dateLimitToString");
var writeUnboundableMessage = /* @__PURE__ */ __name((root) => `Bounded expression ${root} must be exactly one of number, string, Array, or Date`, "writeUnboundableMessage");
...
var implementation3 = implementNode({
  ...
  keys: {
    rule: {
      ...
      serialize: /* @__PURE__ */ __name((schema2) => schema2.toISOString(), "serialize")
    }
  },
  normalize: createDateSchemaNormalizer("after"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `${node2.collapsibleLimitString} or later`, "description"),
    actual: describeCollapsibleDate
  },
  intersections: {
    after: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "after")
  }
});
var _AfterNode = class _AfterNode extends BaseRange {
  constructor() {
    ...
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data) => data >= this.rule, "traverseAllows"));
  }
  reduceJsonSchema(base, ctx) {
    return ctx.fallback.date({ code: "date", base, after: this.rule });
  }
};
__name(_AfterNode, "AfterNode");
...
var implementation4 = implementNode({
  ...
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `${node2.collapsibleLimitString} or earlier`, "description"),
    actual: describeCollapsibleDate
  },
  intersections: {
    before: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "before"),
    after: /* @__PURE__ */ __name((before, after, ctx) => before.overlapsRange(after) ? before.overlapIsUnit(after) ? ctx.$.node("unit", { unit: before.rule }) : null : Disjoint.init("range", before, after), "after")
  }
});
...
var implementation5 = implementNode({
  ...
  hasAssociatedError: true,
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `exactly length ${node2.rule}`, "description"),
    actual: /* @__PURE__ */ __name((data) => `${data.length}`, "actual")
  },
  intersections: {
    exactLength: /* @__PURE__ */ __name((l, r, ctx) => Disjoint.init("unit", ctx.$.node("unit", { unit: l.rule }), ctx.$.node("unit", { unit: r.rule }), { path: ["length"] }), "exactLength"),
    minLength: /* @__PURE__ */ __name((exactLength, minLength) => exactLength.rule >= minLength.rule ? exactLength : Disjoint.init("range", exactLength, minLength), "minLength"),
    maxLength: /* @__PURE__ */ __name((exactLength, maxLength) => exactLength.rule <= maxLength.rule ? exactLength : Disjoint.init("range", exactLength, maxLength), "maxLength")
  }
});
var _ExactLengthNode = class _ExactLengthNode extends InternalPrimitiveConstraint {
  constructor() {
    super(...arguments);
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data) => data.length === this.rule, "traverseAllows"));
...
__name(_ExactLengthNode, "ExactLengthNode");
...
var implementation6 = implementNode({
  ...
  normalize: /* @__PURE__ */ __name((schema2) => typeof schema2 === "number" ? { rule: schema2 } : schema2, "normalize"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => {
      if (node2.rule === 0)
  ...
  intersections: {
    max: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "max"),
    min: /* @__PURE__ */ __name((max, min, ctx) => max.overlapsRange(min) ? max.overlapIsUnit(min) ? ctx.$.node("unit", { unit: max.rule }) : null : Disjoint.init("range", max, min), "min")
  },
  obviatesBasisDescription: true
});
...
var implementation7 = implementNode({
  ...
  reduce: /* @__PURE__ */ __name((inner, $) => inner.rule === 0 ? $.node("exactLength", inner) : void 0, "reduce"),
  normalize: createLengthSchemaNormalizer("maxLength"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `at most length ${node2.rule}`, "description"),
    actual: /* @__PURE__ */ __name((data) => `${data.length}`, "actual")
  },
  intersections: {
    maxLength: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "maxLength"),
    minLength: /* @__PURE__ */ __name((max, min, ctx) => max.overlapsRange(min) ? max.overlapIsUnit(min) ? ctx.$.node("exactLength", { rule: max.rule }) : null : Disjoint.init("range", max, min), "minLength")
  }
});
...
__name(_MaxLengthNode, "MaxLengthNode");
...
var implementation8 = implementNode({
  ...
  intersections: {
    min: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "min")
  },
  obviatesBasisDescription: true
});
...
var implementation9 = implementNode({
  ...
  reduce: /* @__PURE__ */ __name((inner) => inner.rule === 0 ? (
    // a minimum length of zero is trivially satisfied
    $ark.intrinsic.unknown
  ) : void 0, "reduce"),
  normalize: createLengthSchemaNormalizer("minLength"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => node2.rule === 1 ? "non-empty" : `at least length ${node2.rule}`, "description"),
    // avoid default message like "must be non-empty (was 0)"
    actual: /* @__PURE__ */ __name((data) => data.length === 0 ? "" : `${data.length}`, "actual")
  },
  intersections: {
    minLength: /* @__PURE__ */ __name((l, r) => l.isStricterThan(r) ? l : r, "minLength")
  }
});
...
__name(_MinLengthNode, "MinLengthNode");
...
var implementation10 = implementNode({
  ...
  normalize: /* @__PURE__ */ __name((schema2) => typeof schema2 === "string" ? { rule: schema2 } : schema2 instanceof RegExp ? schema2.flags ? { rule: schema2.source, flags: schema2.flags } : { rule: schema2.source } : schema2, "normalize"),
  obviatesBasisDescription: true,
  obviatesBasisExpression: true,
  hasAssociatedError: true,
  intersectionIsOpen: true,
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `matched by ${node2.rule}`, "description")
  },
  intersections: {
    // for now, non-equal regex are naively intersected:
    // https://github.com/arktypeio/arktype/issues/853
    pattern: /* @__PURE__ */ __name(() => null, "pattern")
  }
});
...
__name(_PatternNode, "PatternNode");
...
var schemaKindOf = /* @__PURE__ */ __name((schema2, allowedKinds) => {
...
var discriminateRootKind = /* @__PURE__ */ __name((schema2) => {
...
var writeInvalidSchemaMessage = /* @__PURE__ */ __name((schema2) => `${printable(schema2)} is not a valid type schema`, "writeInvalidSchemaMessage");
var nodeCountsByPrefix = {};
var serializeListableChild = /* @__PURE__ */ __name((listableNode) => isArray2(listableNode) ? listableNode.map((node2) => node2.collapsibleJson) : listableNode.collapsibleJson, "serializeListableChild");
var nodesByRegisteredId = {};
$ark.nodesByRegisteredId = nodesByRegisteredId;
var registerNodeId = /* @__PURE__ */ __name((prefix) => {
  nodeCountsByPrefix[prefix] ??= 0;
  return `${prefix}${++nodeCountsByPrefix[prefix]}`;
}, "registerNodeId");
var parseNode = /* @__PURE__ */ __name((ctx) => {
...
var createNode = /* @__PURE__ */ __name(({ id, kind, inner, meta, $, ignoreCache }) => {
...
var withId = /* @__PURE__ */ __name((node2, id) => {
  if (node2.id === id)
    return node2;
...
var withMeta = /* @__PURE__ */ __name((node2, meta, id) => {
...
var possiblyCollapse = /* @__PURE__ */ __name((json3, toKey, allowPrimitive) => {
...
var intersectProps = /* @__PURE__ */ __name((l, r, ctx) => {
  if (l.key !== r.key)
    return null;
  const key = l.key;
...
var _BaseProp = class _BaseProp extends BaseConstraint {
  constructor() {
    ...
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => {
      if (this.key in data) {
...
__name(_BaseProp, "BaseProp");
var BaseProp = _BaseProp;
var writeDefaultIntersectionMessage = /* @__PURE__ */ __name((lValue, rValue) => `Invalid intersection of default values ${printable(lValue)} & ${printable(rValue)}`, "writeDefaultIntersectionMessage");
...
var implementation11 = implementNode({
  ...
  keys: {
    ...
    value: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => ctx.$.parseSchema(schema2), "parse")
    },
    default: {
      preserveUndefined: true
    }
  },
  normalize: /* @__PURE__ */ __name((schema2) => schema2, "normalize"),
  reduce: /* @__PURE__ */ __name((inner, $) => {
  ...
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `${node2.compiledKey}?: ${node2.value.description}`, "description")
  },
  intersections: {
...
__name(_OptionalNode, "OptionalNode");
...
var getDefaultableMorph = /* @__PURE__ */ __name((node2) => {
  if (!node2.hasDefault())
    return;
...
var computeDefaultValueMorph = /* @__PURE__ */ __name((key, value2, defaultInput) => {
...
var assertDefaultValueAssignability = /* @__PURE__ */ __name((node2, value2, key) => {
...
var writeNonPrimitiveNonFunctionDefaultValueMessage = /* @__PURE__ */ __name((key) => {
...
var _BaseRoot = class _BaseRoot extends BaseNode {
  ...
  get "~standard"() {
    return {
      ...
      toJSONSchema: /* @__PURE__ */ __name((opts) => {
...
`
  ...
  onUndeclaredKey(cfg) {
    ...
    return this.$.finalize(this.transform((kind, inner) => kind === "structure" ? rule === "ignore" ? omit(inner, { undeclared: 1 }) : { ...inner, undeclared: rule } : inner, deep ? void 0 : { shouldTransform: /* @__PURE__ */ __name((node2) => !includes(structuralKinds, node2.kind), "shouldTransform") }));
  }
  hasEqualMorphs(r) {
    ...
    if (!arrayEquals(this.flatMorphs, r.flatMorphs, {
      isEqual: /* @__PURE__ */ __name((l, r2) => l.propString === r2.propString && (l.node.hasKind("morph") && r2.node.hasKind("morph") ? l.node.hasEqualMorphs(r2.node) : l.node.hasKind("intersection") && r2.node.hasKind("intersection") ? l.node.structure?.structuralMorphRef === r2.node.structure?.structuralMorphRef : false), "isEqual")
    }))
      return false;
    return true;
  }
...
__name(_BaseRoot, "BaseRoot");
var BaseRoot = _BaseRoot;
var emptyBrandNameMessage = `Expected a non-empty brand name after #`;
var exclusivizeRangeSchema = /* @__PURE__ */ __name((schema2) => typeof schema2 === "object" && !(schema2 instanceof Date) ? { ...schema2, exclusive: true } : {
  rule: schema2,
  exclusive: true
}, "exclusivizeRangeSchema");
var typeOrTermExtends = /* @__PURE__ */ __name((t, base) => hasArkKind(base, "root") ? hasArkKind(t, "root") ? t.extends(base) : base.allows(t) : hasArkKind(t, "root") ? t.hasUnit(base) : base === t, "typeOrTermExtends");
var structureOf = /* @__PURE__ */ __name((branch) => {
...
var writeLiteralUnionEntriesMessage = /* @__PURE__ */ __name((expression) => `Props cannot be extracted from a union. Use .distribute to extract props from each branch instead. Received:
${expression}`, "writeLiteralUnionEntriesMessage");
var writeNonStructuralOperandMessage = /* @__PURE__ */ __name((operation, operand) => `${operation} operand must be an object (was ${operand})`, "writeNonStructuralOperandMessage");

// node_modules/@ark/schema/out/roots/utils.js
var defineRightwardIntersections = /* @__PURE__ */ __name((kind, implementation23) => flatMorph(schemaKindsRightOf(kind), (i, kind2) => [
  kind2,
  implementation23
]), "defineRightwardIntersections");

// node_modules/@ark/schema/out/roots/alias.js
var normalizeAliasSchema = /* @__PURE__ */ __name((schema2) => typeof schema2 === "string" ? { reference: schema2 } : schema2, "normalizeAliasSchema");
var neverIfDisjoint = /* @__PURE__ */ __name((result) => result instanceof Disjoint ? $ark.intrinsic.never.internal : result, "neverIfDisjoint");
var implementation12 = implementNode({
  ...
  keys: {
    reference: {
      serialize: /* @__PURE__ */ __name((s) => s.startsWith("$") ? s : `$ark.${s}`, "serialize")
    },
    resolve: {}
  },
  normalize: normalizeAliasSchema,
  defaults: {
    description: /* @__PURE__ */ __name((node2) => node2.reference, "description")
  },
  intersections: {
    alias: /* @__PURE__ */ __name((l, r, ctx) => ctx.$.lazilyResolve(() => neverIfDisjoint(intersectOrPipeNodes(l.resolution, r.resolution, ctx)), `${l.reference}${ctx.pipe ? "=>" : "&"}${r.reference}`), "alias"),
...
__name(_AliasNode, "AliasNode");
var AliasNode = _AliasNode;
var writeShallowCycleErrorMessage = /* @__PURE__ */ __name((name, seen) => `Alias '${name}' has a shallow resolution cycle: ${[...seen, name].join("->")}`, "writeShallowCycleErrorMessage");
var Alias = {
...
__name(_InternalBasis, "InternalBasis");
...
var implementation13 = implementNode({
  ...
  normalize: /* @__PURE__ */ __name((schema2) => typeof schema2 === "string" ? { domain: schema2 } : hasKey(schema2, "numberAllowsNaN") && schema2.domain !== "number" ? throwParseError(Domain.writeBadAllowNanMessage(schema2.domain)) : schema2, "normalize"),
  applyConfig: /* @__PURE__ */ __name((schema2, config) => schema2.numberAllowsNaN === void 0 && schema2.domain === "number" && config.numberAllowsNaN ? { ...schema2, numberAllowsNaN: true } : schema2, "applyConfig"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => domainDescriptions[node2.domain], "description"),
    actual: /* @__PURE__ */ __name((data) => Number.isNaN(data) ? "NaN" : domainDescriptions[domainOf(data)], "actual")
  },
  intersections: {
    domain: /* @__PURE__ */ __name((l, r) => (
...
__name(_DomainNode, "DomainNode");
var DomainNode = _DomainNode;
var Domain = {
  implementation: implementation13,
  Node: DomainNode,
  writeBadAllowNanMessage: /* @__PURE__ */ __name((actual) => `numberAllowsNaN may only be specified with domain "number" (was ${actual})`, "writeBadAllowNanMessage")
};
...
var implementation14 = implementNode({
  kind: "intersection",
  hasAssociatedError: true,
  normalize: /* @__PURE__ */ __name((rawSchema) => {
    if (isNode(rawSchema))
      return rawSchema;
  ...
  finalizeInnerJson: /* @__PURE__ */ __name(({ structure, ...rest }) => hasDomain(structure, "object") ? { ...structure, ...rest } : rest, "finalizeInnerJson"),
  keys: {
    domain: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => ctx.$.node("domain", schema2), "parse")
    },
    proto: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => ctx.$.node("proto", schema2), "parse")
    },
    structure: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => ctx.$.node("structure", schema2), "parse"),
      serialize: /* @__PURE__ */ __name((node2) => {
  ...
  defaults: {
    ...
    expected: /* @__PURE__ */ __name((source) => `  \u25E6 ${source.errors.map((e) => e.expected).join("\n  \u25E6 ")}`, "expected"),
    problem: /* @__PURE__ */ __name((ctx) => `(${ctx.actual}) must be...
${ctx.expected}`, "problem")
  },
  intersections: {
    intersection: /* @__PURE__ */ __name((l, r, ctx) => intersectIntersections(l.inner, r.inner, ctx), "intersection"),
...
var _IntersectionNode = class _IntersectionNode extends BaseRoot {
  constructor() {
    ...
    __publicField(this, "prestructurals", []);
    __publicField(this, "refinements", this.children.filter((node2) => {
    ...
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => this.children.every((child) => child.traverseAllows(data, ctx)), "traverseAllows"));
    __publicField(this, "traverseApply", /* @__PURE__ */ __name((data, ctx) => {
...
__name(_IntersectionNode, "IntersectionNode");
var IntersectionNode = _IntersectionNode;
var Intersection = {
  implementation: implementation14,
  Node: IntersectionNode
};
var writeIntersectionExpression = /* @__PURE__ */ __name((node2) => {
...
var intersectIntersections = /* @__PURE__ */ __name((l, r, ctx) => {
  const baseInner = {};
...
var implementation15 = implementNode({
  ...
  keys: {
    ...
    morphs: {
      parse: liftArray,
      serialize: /* @__PURE__ */ __name((morphs) => morphs.map((m) => hasArkKind(m, "root") ? m.json : registeredReference(m)), "serialize")
    },
    declaredIn: {
      child: false,
      serialize: /* @__PURE__ */ __name((node2) => node2.json, "serialize")
    },
    declaredOut: {
      child: false,
      serialize: /* @__PURE__ */ __name((node2) => node2.json, "serialize")
  ...
  normalize: /* @__PURE__ */ __name((schema2) => schema2, "normalize"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `a morph from ${node2.rawIn.description} to ${node2.rawOut?.description ?? "unknown"}`, "description")
  },
  intersections: {
    morph: /* @__PURE__ */ __name((l, r, ctx) => {
...
var _MorphNode = class _MorphNode extends BaseRoot {
  constructor() {
    ...
    __publicField(this, "introspectableOut", this.lastMorphIfNode ? Object.assign(this.referencesById, this.lastMorphIfNode.referencesById) && this.lastMorphIfNode.rawOut : void 0);
    __publicField(this, "expression", `(In: ${this.rawIn.expression}) => ${this.lastMorphIfNode ? "To" : "Out"}<${this.rawOut.expression}>`);
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => !this.introspectableIn || this.introspectableIn.traverseAllows(data, ctx), "traverseAllows"));
    __publicField(this, "traverseApply", /* @__PURE__ */ __name((data, ctx) => {
  ...
  hasEqualMorphs(r) {
    return arrayEquals(this.morphs, r.morphs, {
      isEqual: /* @__PURE__ */ __name((lMorph, rMorph) => lMorph === rMorph || hasArkKind(lMorph, "root") && hasArkKind(rMorph, "root") && lMorph.equals(rMorph), "isEqual")
    });
  }
};
__name(_MorphNode, "MorphNode");
var MorphNode = _MorphNode;
var Morph = {
  implementation: implementation15,
  Node: MorphNode
};
var writeMorphIntersectionMessage = /* @__PURE__ */ __name((lDescription, rDescription) => `The intersection of distinct morphs at a single path is indeterminate:
Left: ${lDescription}
...
var implementation16 = implementNode({
  ...
  keys: {
    proto: {
      serialize: /* @__PURE__ */ __name((ctor) => getBuiltinNameOfConstructor(ctor) ?? defaultValueSerializer(ctor), "serialize")
    },
    dateAllowsInvalid: {}
  },
  normalize: /* @__PURE__ */ __name((schema2) => {
  ...
  applyConfig: /* @__PURE__ */ __name((schema2, config) => {
    if (schema2.dateAllowsInvalid === void 0 && schema2.proto === Date && config.dateAllowsInvalid)
      return { ...schema2, dateAllowsInvalid: true };
    return schema2;
  }, "applyConfig"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => node2.builtinName ? objectKindDescriptions[node2.builtinName] : `an instance of ${node2.proto.name}`, "description"),
    actual: /* @__PURE__ */ __name((data) => data instanceof Date && data.toString() === "Invalid Date" ? "an invalid Date" : objectKindOrDomainOf(data), "actual")
  },
  intersections: {
    proto: /* @__PURE__ */ __name((l, r) => l.proto === Date && r.proto === Date ? (
    ...
    domain: /* @__PURE__ */ __name((proto, domain) => domain.domain === "object" ? proto : Disjoint.init("domain", $ark.intrinsic.object.internal, domain), "domain")
  }
});
...
__name(_ProtoNode, "ProtoNode");
var ProtoNode = _ProtoNode;
var Proto = {
  implementation: implementation16,
  Node: ProtoNode,
  writeBadInvalidDateMessage: /* @__PURE__ */ __name((actual) => `dateAllowsInvalid may only be specified with constructor Date (was ${actual.name})`, "writeBadInvalidDateMessage"),
  writeInvalidSchemaMessage: /* @__PURE__ */ __name((actual) => `instanceOf operand must be a function (was ${domainOf(actual)})`, "writeInvalidSchemaMessage")
};
...
var implementation17 = implementNode({
  ...
  keys: {
    ...
    branches: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => {
        const branches = [];
  ...
  normalize: /* @__PURE__ */ __name((schema2) => isArray2(schema2) ? { branches: schema2 } : schema2, "normalize"),
  reduce: /* @__PURE__ */ __name((inner, $) => {
  ...
  defaults: {
    description: /* @__PURE__ */ __name((node2) => node2.distribute((branch) => branch.description, describeBranches), "description"),
    expected: /* @__PURE__ */ __name((ctx) => {
    ...
    problem: /* @__PURE__ */ __name((ctx) => ctx.expected, "problem"),
...
var _UnionNode = class _UnionNode extends BaseRoot {
  constructor() {
    ...
    __publicField(this, "unitBranches", this.branches.filter((n) => n.rawIn.hasKind("unit")));
    ...
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => this.branches.some((b) => b.traverseAllows(data, ctx)), "traverseAllows"));
    __publicField(this, "traverseApply", /* @__PURE__ */ __name((data, ctx) => {
      const errors = [];
    ...
    __publicField(this, "traverseOptimistic", /* @__PURE__ */ __name((data) => {
      for (let i = 0; i < this.branches.length; i++) {
...
var createCaseResolutionContext = /* @__PURE__ */ __name((viableCandidates, node2) => {
...
var resolveCase = /* @__PURE__ */ __name((ctx, key) => {
...
var viableOrderedCandidates = /* @__PURE__ */ __name((candidates, originalBranches) => {
...
var discriminantCaseToNode = /* @__PURE__ */ __name((caseDiscriminant, path, $) => {
...
var optionallyChainPropString = /* @__PURE__ */ __name((path) => path.reduce((acc, k) => acc + compileLiteralPropAccess(k, true), "data"), "optionallyChainPropString");
...
var discriminantToJson = /* @__PURE__ */ __name((discriminant) => ({
...
var expressBranches = /* @__PURE__ */ __name((expressions) => describeBranches(expressions, describeExpressionOptions), "expressBranches");
var describeBranches = /* @__PURE__ */ __name((descriptions, opts) => {
...
var intersectBranches = /* @__PURE__ */ __name((l, r, ctx) => {
...
var reduceBranches = /* @__PURE__ */ __name(({ branches, ordered }) => {
...
var assertDeterminateOverlap = /* @__PURE__ */ __name((l, r) => {
  ...
  if (!arrayEquals(l.flatMorphs, r.flatMorphs, {
    isEqual: /* @__PURE__ */ __name((l2, r2) => l2.propString === r2.propString && (l2.node.hasKind("morph") && r2.node.hasKind("morph") ? l2.node.hasEqualMorphs(r2.node) : l2.node.hasKind("intersection") && r2.node.hasKind("intersection") ? l2.node.structure?.structuralMorphRef === r2.node.structure?.structuralMorphRef : false), "isEqual")
  })) {
    throwParseError(writeIndiscriminableMorphMessage(l.expression, r.expression));
  }
}, "assertDeterminateOverlap");
var pruneDiscriminant = /* @__PURE__ */ __name((discriminantBranch, discriminantCtx) => discriminantBranch.transform((nodeKind, inner) => {
  if (nodeKind === "domain" || nodeKind === "unit")
    return null;
  return inner;
}, {
  shouldTransform: /* @__PURE__ */ __name((node2, ctx) => {
...
var writeIndiscriminableMorphMessage = /* @__PURE__ */ __name((lDescription, rDescription) => `An unordered union of a type including a morph and a type with overlapping input is indeterminate:
Left: ${lDescription}
Right: ${rDescription}`, "writeIndiscriminableMorphMessage");
var writeOrderedIntersectionMessage = /* @__PURE__ */ __name((lDescription, rDescription) => `The intersection of two ordered unions is indeterminate:
Left: ${lDescription}
...
var implementation18 = implementNode({
  ...
  keys: {
    unit: {
      preserveUndefined: true,
      serialize: /* @__PURE__ */ __name((schema2) => schema2 instanceof Date ? schema2.toISOString() : defaultValueSerializer(schema2), "serialize")
    }
  },
  normalize: /* @__PURE__ */ __name((schema2) => schema2, "normalize"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => printable(node2.unit), "description"),
    problem: /* @__PURE__ */ __name(({ expected, actual }) => `${expected === actual ? `must be reference equal to ${expected} (serialized to the same value)` : `must be ${expected} (was ${actual})`}`, "problem")
  },
  intersections: {
    unit: /* @__PURE__ */ __name((l, r) => Disjoint.init("unit", l, r), "unit"),
...
__name(_UnitNode, "UnitNode");
var UnitNode = _UnitNode;
var Unit = {
  implementation: implementation18,
  Node: UnitNode
};
var compileEqualityCheck = /* @__PURE__ */ __name((unit, serializedValue, negated) => {
...
var implementation19 = implementNode({
  ...
  normalize: /* @__PURE__ */ __name((schema2) => schema2, "normalize"),
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `[${node2.signature.expression}]: ${node2.value.description}`, "description")
  },
  intersections: {
    index: /* @__PURE__ */ __name((l, r, ctx) => {
...
var _IndexNode = class _IndexNode extends BaseConstraint {
  constructor() {
    ...
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => stringAndSymbolicEntriesOf(data).every((entry) => {
...
__name(_IndexNode, "IndexNode");
var IndexNode = _IndexNode;
var Index = {
  implementation: implementation19,
  Node: IndexNode
};
var writeEnumerableIndexBranches = /* @__PURE__ */ __name((keys) => `Index keys ${keys.join(", ")} should be specified as named props.`, "writeEnumerableIndexBranches");
var writeInvalidPropertyKeyMessage = /* @__PURE__ */ __name((indexSchema) => `Indexed key definition '${indexSchema}' must be a string or symbol`, "writeInvalidPropertyKeyMessage");
...
var implementation20 = implementNode({
  ...
  defaults: {
    description: /* @__PURE__ */ __name((node2) => `${node2.compiledKey}: ${node2.value.description}`, "description"),
    expected: /* @__PURE__ */ __name((ctx) => ctx.missingValueDescription, "expected"),
    actual: /* @__PURE__ */ __name(() => "missing", "actual")
  },
  intersections: {
...
__name(_RequiredNode, "RequiredNode");
...
var implementation21 = implementNode({
  ...
  keys: {
    ...
    defaultables: {
      child: /* @__PURE__ */ __name((defaultables) => defaultables.map((element) => element[0]), "child"),
      parse: /* @__PURE__ */ __name((defaultables, ctx) => {
      ...
      serialize: /* @__PURE__ */ __name((defaults) => defaults.map((element) => [
        element[0].collapsibleJson,
        defaultValueSerializer(element[1])
      ]), "serialize"),
      reduceIo: /* @__PURE__ */ __name((ioKind, inner, defaultables) => {
        if (ioKind === "in") {
    ...
    variadic: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => ctx.$.parseSchema(schema2, ctx), "parse")
    },
    minVariadicLength: {
      // minVariadicLength is reflected in the id of this node,
      // but not its IntersectionNode parent since it is superceded by the minLength
      // node it implies
      parse: /* @__PURE__ */ __name((min) => min === 0 ? void 0 : min, "parse")
    },
    postfix: {
      child: true,
      parse: /* @__PURE__ */ __name((schema2, ctx) => {
  ...
  intersections: {
    sequence: /* @__PURE__ */ __name((l, r, ctx) => {
...
var _SequenceNode = class _SequenceNode extends BaseConstraint {
  constructor() {
    ...
    __publicField(this, "prefixLength", this.prefix?.length ?? 0);
    __publicField(this, "defaultablesLength", this.defaultables?.length ?? 0);
    __publicField(this, "optionalsLength", this.optionals?.length ?? 0);
    __publicField(this, "postfixLength", this.postfix?.length ?? 0);
...
__name(_SequenceNode, "SequenceNode");
var SequenceNode = _SequenceNode;
var defaultableMorphsCache = {};
var getDefaultableMorphs = /* @__PURE__ */ __name((node2) => {
...
var sequenceInnerToTuple = /* @__PURE__ */ __name((inner) => {
  const tuple = [];
  if (inner.prefix)
...
var sequenceTupleToInner = /* @__PURE__ */ __name((tuple) => tuple.reduce((result, element) => {
...
var _intersectSequences = /* @__PURE__ */ __name((s) => {
...
var elementIsRequired = /* @__PURE__ */ __name((el) => el.kind === "prefix" || el.kind === "postfix", "elementIsRequired");

// node_modules/@ark/schema/out/structure/structure.js
var createStructuralWriter = /* @__PURE__ */ __name((childStringProp) => (node2) => {
...
var intersectPropsAndIndex = /* @__PURE__ */ __name((l, r, $) => {
...
var implementation22 = implementNode({
  ...
  keys: {
    required: {
      ...
      reduceIo: /* @__PURE__ */ __name((ioKind, inner, nodes) => {
    ...
    undeclared: {
      parse: /* @__PURE__ */ __name((behavior) => behavior === "ignore" ? void 0 : behavior, "parse"),
      reduceIo: /* @__PURE__ */ __name((ioKind, inner, value2) => {
        if (value2 === "reject") {
  ...
  intersections: {
    structure: /* @__PURE__ */ __name((l, r, ctx) => {
...
var _StructureNode = class _StructureNode extends BaseConstraint {
  constructor() {
    ...
    __publicField(this, "requiredKeys", this.required?.map((node2) => node2.key) ?? []);
    __publicField(this, "optionalKeys", this.optional?.map((node2) => node2.key) ?? []);
    __publicField(this, "literalKeys", [...this.requiredKeys, ...this.optionalKeys]);
    __publicField(this, "_keyof");
    __publicField(this, "traverseAllows", /* @__PURE__ */ __name((data, ctx) => this._traverse("Allows", data, ctx), "traverseAllows"));
    __publicField(this, "traverseApply", /* @__PURE__ */ __name((data, ctx) => this._traverse("Apply", data, ctx), "traverseApply"));
    __publicField(this, "_traverse", /* @__PURE__ */ __name((traversalKind, data, ctx) => {
    ...
    __publicField(this, "declaresKey", /* @__PURE__ */ __name((k) => k in this.propsByKey || this.index?.some((n) => n.signature.allows(k)) || this.sequence !== void 0 && $ark.intrinsic.nonNegativeIntegerString.allows(k), "declaresKey"));
  ...
  filterKeys(operation, keys) {
    ...
    const shouldKeep = /* @__PURE__ */ __name((key) => {
...
__name(_StructureNode, "StructureNode");
var StructureNode = _StructureNode;
var defaultableMorphsCache2 = {};
var constructStructuralMorphCacheKey = /* @__PURE__ */ __name((node2) => {
  let cacheKey = "";
...
var getPossibleMorph = /* @__PURE__ */ __name((node2) => {
  ...
  const $arkStructuralMorph = /* @__PURE__ */ __name((data, ctx) => {
    for (let i = 0; i < node2.defaultable.length; i++) {
...
var precompileMorphs = /* @__PURE__ */ __name((js, node2) => {
...
var indexerToKey = /* @__PURE__ */ __name((indexable) => {
...
var writeNumberIndexMessage = /* @__PURE__ */ __name((indexExpression, sequenceExpression) => `${indexExpression} is not allowed as an array index on ${sequenceExpression}. Use the 'nonNegativeIntegerString' keyword instead.`, "writeNumberIndexMessage");
var normalizeIndex = /* @__PURE__ */ __name((signature, value2, $) => {
...
var typeKeyToString = /* @__PURE__ */ __name((k) => hasArkKind(k, "root") ? k.expression : printable(k), "typeKeyToString");
var writeInvalidKeysMessage = /* @__PURE__ */ __name((o, keys) => `Key${keys.length === 1 ? "" : "s"} ${keys.map(typeKeyToString).join(", ")} ${keys.length === 1 ? "does" : "do"} not exist on ${o}`, "writeInvalidKeysMessage");
var writeDuplicateKeyMessage = /* @__PURE__ */ __name((key) => `Duplicate key ${compileSerializedValue(key)}`, "writeDuplicateKeyMessage");
...
__name(_RootModule, "RootModule");
var RootModule = _RootModule;
var bindModule = /* @__PURE__ */ __name((module, $) => new RootModule(flatMorph(module, (alias, value2) => [
  alias,
  hasArkKind(value2, "module") ? bindModule(value2, $) : $.bindReference(value2)
])), "bindModule");

// node_modules/@ark/schema/out/scope.js
var schemaBranchesOf = /* @__PURE__ */ __name((schema2) => isArray2(schema2) ? schema2 : "branches" in schema2 && isArray2(schema2.branches) ? schema2.branches : void 0, "schemaBranchesOf");
var throwMismatchedNodeRootError = /* @__PURE__ */ __name((expected, actual) => throwParseError(`Node of kind ${actual} is not valid as a ${expected} definition`), "throwMismatchedNodeRootError");
var writeDuplicateAliasError = /* @__PURE__ */ __name((alias) => `#${alias} duplicates public alias ${alias}`, "writeDuplicateAliasError");
var scopesByName = {};
$ark.ambient ??= {};
var rawUnknownUnion;
var rootScopeFnName = "function $";
var precompile = /* @__PURE__ */ __name((references) => bindPrecompilation(references, precompileReferences(references)), "precompile");
var bindPrecompilation = /* @__PURE__ */ __name((references, precompiler) => {
...
var precompileReferences = /* @__PURE__ */ __name((references) => new CompiledFunction().return(references.reduce((js, node2) => {
...
var _BaseScope = class _BaseScope {
  constructor(def, config) {
    ...
    __publicField(this, "schema", /* @__PURE__ */ __name((schema2, opts) => this.finalize(this.parseSchema(schema2, opts)), "schema"));
    __publicField(this, "parseSchema", /* @__PURE__ */ __name((schema2, opts) => this.node(schemaKindOf(schema2), schema2, opts), "parseSchema"));
    __publicField(this, "precompilation");
    __publicField(this, "_exportedResolutions");
    __publicField(this, "_exports");
    __publicField(this, "node", /* @__PURE__ */ __name((kinds, nodeSchema, opts = {}) => {
    ...
    __publicField(this, "parse", /* @__PURE__ */ __name((def, opts = {}) => this.finalize(this.parseDefinition(def, opts)), "parse"));
...
__name(_SchemaScope, "SchemaScope");
var SchemaScope = _SchemaScope;
var bootstrapAliasReferences = /* @__PURE__ */ __name((resolution) => {
...
var resolutionsToJson = /* @__PURE__ */ __name((resolutions) => flatMorph(resolutions, (k, v) => [
  k,
...
var maybeResolveSubalias = /* @__PURE__ */ __name((base, name) => {
...
var schemaScope = /* @__PURE__ */ __name((aliases, config) => new SchemaScope(aliases, config), "schemaScope");
var rootSchemaScope = new SchemaScope({});
var resolutionsOfModule = /* @__PURE__ */ __name(($, typeSet) => {
  const result = {};
...
var writeUnresolvableMessage = /* @__PURE__ */ __name((token) => `'${token}' is unresolvable`, "writeUnresolvableMessage");
var writeNonSubmoduleDotMessage = /* @__PURE__ */ __name((name) => `'${name}' must reference a module to be accessed using dot syntax`, "writeNonSubmoduleDotMessage");
var writeMissingSubmoduleAccessMessage = /* @__PURE__ */ __name((name) => `Reference to submodule '${name}' must specify an alias`, "writeMissingSubmoduleAccessMessage");
...
var regex = /* @__PURE__ */ __name((src, flags) => new RegExp(src, flags), "regex");
Object.assign(regex, { as: regex });

// node_modules/arktype/out/parser/shift/operand/date.js
var isDateLiteral = /* @__PURE__ */ __name((value2) => typeof value2 === "string" && value2[0] === "d" && (value2[1] === "'" || value2[1] === '"') && value2.at(-1) === value2[1], "isDateLiteral");
var isValidDate = /* @__PURE__ */ __name((d) => d.toString() !== "Invalid Date", "isValidDate");
var extractDateLiteralSource = /* @__PURE__ */ __name((literal) => literal.slice(2, -1), "extractDateLiteralSource");
var writeInvalidDateMessage = /* @__PURE__ */ __name((source) => `'${source}' could not be parsed by the Date constructor`, "writeInvalidDateMessage");
var tryParseDate = /* @__PURE__ */ __name((source, errorOnFail) => maybeParseDate(source, errorOnFail), "tryParseDate");
var maybeParseDate = /* @__PURE__ */ __name((source, errorOnFail) => {
...
var parseEnclosed = /* @__PURE__ */ __name((s, enclosing) => {
...
var untilLookaheadIsClosing = {
  "'": /* @__PURE__ */ __name((scanner) => scanner.lookahead === `'`, "'"),
  '"': /* @__PURE__ */ __name((scanner) => scanner.lookahead === `"`, '"'),
  "/": /* @__PURE__ */ __name((scanner) => scanner.lookahead === `/`, "/")
};
...
var writeUnterminatedEnclosedMessage = /* @__PURE__ */ __name((fragment, enclosingStart) => `${enclosingStart}${fragment} requires a closing ${enclosingCharDescriptions[enclosingTokens[enclosingStart]]}`, "writeUnterminatedEnclosedMessage");

// node_modules/arktype/out/parser/ast/validate.js
var writePrefixedPrivateReferenceMessage = /* @__PURE__ */ __name((name) => `Private type references should not include '#'. Use '${name}' instead.`, "writePrefixedPrivateReferenceMessage");
...
var lookaheadIsFinalizing = /* @__PURE__ */ __name((lookahead, unscanned) => lookahead === ">" ? unscanned[0] === "=" ? (
...
var parseGenericArgs = /* @__PURE__ */ __name((name, g, s) => _parseGenericArgs(name, g, s, []), "parseGenericArgs");
var _parseGenericArgs = /* @__PURE__ */ __name((name, g, s, argNodes) => {
...
var writeInvalidGenericArgCountMessage = /* @__PURE__ */ __name((name, params, argDefs) => `${name}<${params.join(", ")}> requires exactly ${params.length} args (got ${argDefs.length}${argDefs.length === 0 ? "" : `: ${argDefs.join(", ")}`})`, "writeInvalidGenericArgCountMessage");

// node_modules/arktype/out/parser/shift/operand/unenclosed.js
var parseUnenclosed = /* @__PURE__ */ __name((s) => {
...
var parseGenericInstantiation = /* @__PURE__ */ __name((name, g, s) => {
...
var maybeParseUnenclosedLiteral = /* @__PURE__ */ __name((s, token) => {
...
var writeMissingOperandMessage = /* @__PURE__ */ __name((s) => {
...
var writeOpenRangeMessage = /* @__PURE__ */ __name((min, comparator) => `Left bounds are only valid when paired with right bounds (try ...${comparator}${min})`, "writeOpenRangeMessage");
var writeUnpairableComparatorMessage = /* @__PURE__ */ __name((comparator) => `Left-bounded expressions must specify their limits using < or <= (was ${comparator})`, "writeUnpairableComparatorMessage");
var writeMultipleLeftBoundsMessage = /* @__PURE__ */ __name((openLimit, openComparator, limit, comparator) => `An expression may have at most one left bound (parsed ${openLimit}${invertedComparators[openComparator]}, ${limit}${invertedComparators[comparator]})`, "writeMultipleLeftBoundsMessage");

// node_modules/arktype/out/parser/shift/operator/bounds.js
var parseBound = /* @__PURE__ */ __name((s, start) => {
...
var shiftComparator = /* @__PURE__ */ __name((s, start) => s.scanner.lookaheadIs("=") ? `${start}${s.scanner.shift()}` : start, "shiftComparator");
var getBoundKinds = /* @__PURE__ */ __name((comparator, limit, root, boundKind) => {
...
var openLeftBoundToRoot = /* @__PURE__ */ __name((leftBound) => ({
...
var parseRightBound = /* @__PURE__ */ __name((s, comparator) => {
...
var writeInvalidLimitMessage = /* @__PURE__ */ __name((comparator, limit, boundKind) => `Comparator ${boundKind === "left" ? invertedComparators[comparator] : comparator} must be ${boundKind === "left" ? "preceded" : "followed"} by a corresponding literal (was ${limit})`, "writeInvalidLimitMessage");

// node_modules/arktype/out/parser/shift/operator/brand.js
var parseBrand = /* @__PURE__ */ __name((s) => {
  s.scanner.shiftUntilNonWhitespace();
...
var parseDivisor = /* @__PURE__ */ __name((s) => {
  s.scanner.shiftUntilNonWhitespace();
...
var writeInvalidDivisorMessage = /* @__PURE__ */ __name((divisor) => `% operator must be followed by a non-zero integer literal (was ${divisor})`, "writeInvalidDivisorMessage");

// node_modules/arktype/out/parser/shift/operator/operator.js
var parseOperator = /* @__PURE__ */ __name((s) => {
  const lookahead = s.scanner.shift();
...
var writeUnexpectedCharacterMessage = /* @__PURE__ */ __name((char, shouldBe = "") => `'${char}' is not allowed here${shouldBe && ` (should be ${shouldBe})`}`, "writeUnexpectedCharacterMessage");
var incompleteArrayTokenMessage = `Missing expected ']'`;

// node_modules/arktype/out/parser/shift/operator/default.js
var parseDefault = /* @__PURE__ */ __name((s) => {
...
var writeNonLiteralDefaultMessage = /* @__PURE__ */ __name((defaultDef) => `Default value '${defaultDef}' must be a literal value`, "writeNonLiteralDefaultMessage");

// node_modules/arktype/out/parser/string.js
var parseString = /* @__PURE__ */ __name((def, ctx) => {
...
var fullStringParse = /* @__PURE__ */ __name((s) => {
  s.parseOperand();
...
var parseUntilFinalizer = /* @__PURE__ */ __name((s) => {
  while (s.finalizer === void 0)
    next(s);
  return s;
}, "parseUntilFinalizer");
var next = /* @__PURE__ */ __name((s) => s.hasRoot() ? s.parseOperator() : s.parseOperand(), "next");
...
__name(_RuntimeState, "RuntimeState");
...
var parseGenericParamName = /* @__PURE__ */ __name((scanner, result, ctx) => {
...
var _parseOptionalConstraint = /* @__PURE__ */ __name((scanner, name, result, ctx) => {
  scanner.shiftUntilNonWhitespace();
...
__name(_InternalFnParser, "InternalFnParser");
...
__name(_InternalTypedFn, "InternalTypedFn");
...
__name(_InternalMatchParser, "InternalMatchParser");
...
__name(_InternalChainedMatchParser, "InternalChainedMatchParser");
var InternalChainedMatchParser = _InternalChainedMatchParser;
var throwOnDefault = /* @__PURE__ */ __name((errors) => errors.throw(), "throwOnDefault");
...
var parseProperty = /* @__PURE__ */ __name((def, ctx) => {
  if (isArray2(def)) {
    if (def[1] === "=")
...
var parseObjectLiteral = /* @__PURE__ */ __name((def, ctx) => {
  let spread;
  const structure = {};
...
var appendNamedProp = /* @__PURE__ */ __name((structure, kind, inner, ctx) => {
...
var writeInvalidUndeclaredBehaviorMessage = /* @__PURE__ */ __name((actual) => `Value of '+' key must be 'reject', 'delete', or 'ignore' (was ${printable(actual)})`, "writeInvalidUndeclaredBehaviorMessage");
var nonLeadingSpreadError = "Spread operator may only be used as the first key in an object";
var preparseKey = /* @__PURE__ */ __name((key) => typeof key === "symbol" ? { kind: "required", normalized: key } : key.at(-1) === "?" ? key.at(-2) === Backslash ? { kind: "required", normalized: `${key.slice(0, -2)}?` } : {
  kind: "optional",
...
var writeInvalidSpreadTypeMessage = /* @__PURE__ */ __name((def) => `Spread operand must resolve to an object literal type (was ${def})`, "writeInvalidSpreadTypeMessage");

// node_modules/arktype/out/parser/tupleExpressions.js
var maybeParseTupleExpression = /* @__PURE__ */ __name((def, ctx) => isIndexZeroExpression(def) ? indexZeroParsers[def[0]](def, ctx) : isIndexOneExpression(def) ? indexOneParsers[def[1]](def, ctx) : null, "maybeParseTupleExpression");
var parseKeyOfTuple = /* @__PURE__ */ __name((def, ctx) => ctx.$.parseOwnDefinitionFormat(def[1], ctx).keyof(), "parseKeyOfTuple");
var parseBranchTuple = /* @__PURE__ */ __name((def, ctx) => {
  if (def[2] === void 0)
...
var parseArrayTuple = /* @__PURE__ */ __name((def, ctx) => ctx.$.parseOwnDefinitionFormat(def[0], ctx).array(), "parseArrayTuple");
var parseMorphTuple = /* @__PURE__ */ __name((def, ctx) => {
...
var writeMalformedFunctionalExpressionMessage = /* @__PURE__ */ __name((operator, value2) => `${operator === ":" ? "Narrow" : "Morph"} expression requires a function following '${operator}' (was ${typeof value2})`, "writeMalformedFunctionalExpressionMessage");
var parseNarrowTuple = /* @__PURE__ */ __name((def, ctx) => {
  if (typeof def[2] !== "function") {
    return throwParseError(writeMalformedFunctionalExpressionMessage(":", def[2]));
  }
  return ctx.$.parseOwnDefinitionFormat(def[0], ctx).constrain("predicate", def[2]);
}, "parseNarrowTuple");
var parseMetaTuple = /* @__PURE__ */ __name((def, ctx) => ctx.$.parseOwnDefinitionFormat(def[0], ctx).configure(def[2], def[3]), "parseMetaTuple");
var defineIndexOneParsers = /* @__PURE__ */ __name((parsers) => parsers, "defineIndexOneParsers");
var postfixParsers = defineIndexOneParsers({
  "[]": parseArrayTuple,
  "?": /* @__PURE__ */ __name(() => throwParseError(shallowOptionalMessage), "?")
});
var infixParsers = defineIndexOneParsers({
  ...
  "=": /* @__PURE__ */ __name(() => throwParseError(shallowDefaultableMessage), "=")
});
var indexOneParsers = { ...postfixParsers, ...infixParsers };
var isIndexOneExpression = /* @__PURE__ */ __name((def) => indexOneParsers[def[1]] !== void 0, "isIndexOneExpression");
var defineIndexZeroParsers = /* @__PURE__ */ __name((parsers) => parsers, "defineIndexZeroParsers");
var indexZeroParsers = defineIndexZeroParsers({
  keyof: parseKeyOfTuple,
  instanceof: /* @__PURE__ */ __name((def, ctx) => {
  ...
  "===": /* @__PURE__ */ __name((def, ctx) => ctx.$.units(def.slice(1)), "===")
});
var isIndexZeroExpression = /* @__PURE__ */ __name((def) => indexZeroParsers[def[0]] !== void 0, "isIndexZeroExpression");
var writeInvalidConstructorMessage = /* @__PURE__ */ __name((actual) => `Expected a constructor following 'instanceof' operator (was ${actual})`, "writeInvalidConstructorMessage");

// node_modules/arktype/out/parser/tupleLiteral.js
var parseTupleLiteral = /* @__PURE__ */ __name((def, ctx) => {
  let sequences = [{}];
  let i = 0;
  while (i < def.length) {
    let spread = false;
...
var appendRequiredElement = /* @__PURE__ */ __name((base, element) => {
...
var appendOptionalElement = /* @__PURE__ */ __name((base, element) => {
  if (base.variadic)
...
var appendDefaultableElement = /* @__PURE__ */ __name((base, element, value2) => {
  if (base.variadic)
    return throwParseError(optionalOrDefaultableAfterVariadicMessage);
  if (base.optionals)
...
var appendVariadicElement = /* @__PURE__ */ __name((base, element) => {
  if (base.postfix)
...
var appendSpreadBranch = /* @__PURE__ */ __name((base, branch) => {
...
var writeNonArraySpreadMessage = /* @__PURE__ */ __name((operand) => `Spread element must be an array (was ${operand})`, "writeNonArraySpreadMessage");
...
var parseCache = {};
var parseInnerDefinition = /* @__PURE__ */ __name((def, ctx) => {
...
var parseObject = /* @__PURE__ */ __name((def, ctx) => {
...
var parseTuple = /* @__PURE__ */ __name((def, ctx) => maybeParseTupleExpression(def, ctx) ?? parseTupleLiteral(def, ctx), "parseTuple");
var writeBadDefinitionTypeMessage = /* @__PURE__ */ __name((actual) => `Type definitions must be strings or objects (was ${actual})`, "writeBadDefinitionTypeMessage");
...
__name(_InternalTypeParser, "InternalTypeParser");
...
var _InternalScope = class _InternalScope extends BaseScope {
  constructor() {
    super(...arguments);
    __publicField(this, "unit", /* @__PURE__ */ __name((value2) => this.units([value2]), "unit"));
    __publicField(this, "valueOf", /* @__PURE__ */ __name((tsEnum) => this.units(enumValues(tsEnum)), "valueOf"));
    __publicField(this, "enumerated", /* @__PURE__ */ __name((...values) => this.units(values), "enumerated"));
    __publicField(this, "instanceOf", /* @__PURE__ */ __name((ctor) => this.node("proto", { proto: ctor }, { prereduced: true }), "instanceOf"));
    __publicField(this, "or", /* @__PURE__ */ __name((...defs) => this.schema(defs.map((def) => this.parse(def))), "or"));
    __publicField(this, "and", /* @__PURE__ */ __name((...defs) => defs.reduce((node2, def) => node2.and(this.parse(def)), this.intrinsic.unknown), "and"));
    __publicField(this, "merge", /* @__PURE__ */ __name((...defs) => defs.reduce((node2, def) => node2.merge(this.parse(def)), this.intrinsic.object), "merge"));
    __publicField(this, "pipe", /* @__PURE__ */ __name((...morphs) => this.intrinsic.unknown.pipe(...morphs), "pipe"));
    __publicField(this, "fn", new InternalFnParser(this));
    __publicField(this, "match", new InternalMatchParser(this));
    __publicField(this, "declare", /* @__PURE__ */ __name(() => ({
      type: this.type
    }), "declare"));
...
__name(_InternalScope, "InternalScope");
__publicField(_InternalScope, "scope", /* @__PURE__ */ __name((def, config = {}) => new _InternalScope(def, config), "scope"));
__publicField(_InternalScope, "module", /* @__PURE__ */ __name((def, config = {}) => _InternalScope.scope(def, config).export(), "module"));
var InternalScope = _InternalScope;
var scope = Object.assign(InternalScope.scope, {
  define: /* @__PURE__ */ __name((def) => def, "define")
});
...
__name(_liftFromHkt, "liftFromHkt");
...
var regexStringNode = /* @__PURE__ */ __name((regex3, description, jsonSchemaFormat) => {
  const schema2 = {
    domain: "string",
    pattern: {
      rule: regex3.source,
...
var capitalize2 = Scope.module({
  root: rootSchema({
    in: "string",
    morphs: /* @__PURE__ */ __name((s) => s.charAt(0).toUpperCase() + s.slice(1), "morphs"),
...
var isLuhnValid = /* @__PURE__ */ __name((creditCardInput) => {
...
var isParsableDate = /* @__PURE__ */ __name((s) => !Number.isNaN(new Date(s).valueOf()), "isParsableDate");
...
var epoch2 = Scope.module({
  ...
  parse: rootSchema({
    in: epochRoot,
    morphs: /* @__PURE__ */ __name((s) => new Date(s), "morphs"),
...
var writeJsonSyntaxErrorProblem = /* @__PURE__ */ __name((error) => {
...
var jsonRoot = rootSchema({
  ...
  predicate: {
    meta: jsonStringDescription,
    predicate: /* @__PURE__ */ __name((s, ctx) => {
      try {
        JSON.parse(s);
        return true;
      } catch (e) {
...
var parseJson = /* @__PURE__ */ __name((s, ctx) => {
  if (s.length === 0) {
    return ctx.error({
      code: "predicate",
      expected: jsonStringDescription,
...
var lower = Scope.module({
  root: rootSchema({
    in: "string",
    morphs: /* @__PURE__ */ __name((s) => s.toLowerCase(), "morphs"),
...
var preformattedNodes = flatMorph(normalizedForms, (i, form) => [
  ...
  rootSchema({
    domain: "string",
    predicate: /* @__PURE__ */ __name((s) => s.normalize(form) === s, "predicate"),
...
var normalizeNodes = flatMorph(normalizedForms, (i, form) => [
  ...
  rootSchema({
    in: "string",
    morphs: /* @__PURE__ */ __name((s) => s.normalize(form), "morphs"),
...
var stringNumeric = Scope.module({
  ...
  parse: rootSchema({
    in: numericRoot,
    morphs: /* @__PURE__ */ __name((s) => Number.parseFloat(s), "morphs"),
...
var trim = Scope.module({
  root: rootSchema({
    in: "string",
    morphs: /* @__PURE__ */ __name((s) => s.trim(), "morphs"),
...
var upper = Scope.module({
  root: rootSchema({
    in: "string",
    morphs: /* @__PURE__ */ __name((s) => s.toUpperCase(), "morphs"),
...
var isParsableUrl = /* @__PURE__ */ __name((s) => URL.canParse(s), "isParsableUrl");
...
var json2 = Scope.module({
  ...
  stringify: node("morph", {
    ...
    morphs: /* @__PURE__ */ __name((data) => JSON.stringify(data), "morphs"),
...
__name(_RequiredHkt, "RequiredHkt");
...
var selectTheme = /* @__PURE__ */ __name((themes, themeName) => {
  return themes.find((theme) => theme.name === themeName);
}, "selectTheme");
var resolveThemes = /* @__PURE__ */ __name((jsonFiles) => {
...
var extractSetNames = /* @__PURE__ */ __name((selectedTokenSets) => {
...
__name(determineSets, "determineSets");
...
__name(getSetTokens, "getSetTokens");
...
__name(flattenToTokens, "flattenToTokens");
...
__name(processTokenSets, "processTokenSets");
...
var specName = /* @__PURE__ */ __name((spec) => spec.name.toLowerCase(), "specName");
...
__name(fetchTokenScriptSchema, "fetchTokenScriptSchema");
...
__name(fetchAndRegisterSchemas, "fetchAndRegisterSchemas");
...
__name(parseSemverFromString, "parseSemverFromString");
function parseVersionString(versionString) {
  return versionString === "latest" ? "latest" : parseSemverFromString(versionString);
}
__name(parseVersionString, "parseVersionString");
...
__name(semverToString, "semverToString");
...
__name(buildSchemaUri, "buildSchemaUri");
...
__name(parseSchemaUri, "parseSchemaUri");
...
__name(parseSemverFromUri, "parseSemverFromUri");
...
__name(generateVersionCandidates, "generateVersionCandidates");
...
__name(isSameSchema, "isSameSchema");
...
var _ColorManager = class _ColorManager extends BaseManager {
  ...
  registerRootInitializers(_uri, spec) {
    spec.initializers.forEach((spec2) => {
      try {
        ...
        const fn2 = /* @__PURE__ */ __name((args2) => {
  ...
  registerConversions(uri, spec) {
    spec.conversions.forEach((conversion) => {
      ...
      const fn2 = /* @__PURE__ */ __name((color) => {
        ...
        if (!(result instanceof ColorSymbol)) {
          ...
          return new ColorSymbol(value2, targetSpec.name, null, this.parentConfig);
        }
        return result;
      }, "fn");
...
__name(extractNumber, "extractNumber");
...
function overMany(args2, fn2, functionName, minArgs) {
  ...
  const firstUnitArg = args2.find((arg) => arg instanceof NumberWithUnitSymbol);
  if (firstUnitArg) {
    return new NumberWithUnitSymbol(result, firstUnitArg.unit);
  }
  return new NumberSymbol(result);
}
__name(overMany, "overMany");
function createSumFunction(getUnitManager) {
  return (...args2) => {
    ...
    const hasUnits = args2.some((arg) => arg instanceof NumberWithUnitSymbol);
    if (!hasUnits) {
...
__name(createSumFunction, "createSumFunction");
var mathFunctions = {
  min: /* @__PURE__ */ __name((...args2) => overMany(args2, (nums) => Math.min(...nums), "min", 1), "min"),
  max: /* @__PURE__ */ __name((...args2) => overMany(args2, (nums) => Math.max(...nums), "max", 1), "max"),
  mod: /* @__PURE__ */ __name((a, b) => overTwo(
    a,
    b,
    (aVal, bVal) => {
      if (bVal === 0)
  ...
  average: /* @__PURE__ */ __name((...args2) => overMany(args2, (nums) => nums.reduce((acc, val) => acc + val, 0) / nums.length, "average", 1), "average"),
  round: /* @__PURE__ */ __name((arg) => over(arg, Math.round, "round"), "round"),
  abs: /* @__PURE__ */ __name((arg) => over(arg, Math.abs, "abs"), "abs"),
  sqrt: /* @__PURE__ */ __name((arg) => over(arg, Math.sqrt, "sqrt"), "sqrt"),
  floor: /* @__PURE__ */ __name((arg) => over(arg, Math.floor, "floor"), "floor"),
  ceil: /* @__PURE__ */ __name((arg) => over(arg, Math.ceil, "ceil"), "ceil"),
  cbrt: /* @__PURE__ */ __name((arg) => over(arg, Math.cbrt, "cbrt"), "cbrt"),
  sign: /* @__PURE__ */ __name((arg) => new NumberSymbol(Math.sign(extractNumber(arg, "sign"))), "sign"),
  trunc: /* @__PURE__ */ __name((arg) => over(arg, Math.trunc, "trunc"), "trunc"),
  exp: /* @__PURE__ */ __name((arg) => over(arg, Math.exp, "exp"), "exp"),
  expm1: /* @__PURE__ */ __name((arg) => over(arg, Math.expm1, "expm1"), "expm1"),
  pow: /* @__PURE__ */ __name((base, exp) => overTwo(base, exp, (b, e) => b ** e, "pow"), "pow"),
  round_to: /* @__PURE__ */ __name((value2, precision) => {
  ...
  hypot: /* @__PURE__ */ __name((...args2) => overMany(args2, (nums) => Math.hypot(...nums), "hypot", 1), "hypot"),
  remainder: /* @__PURE__ */ __name((a, b) => overTwo(
    a,
    b,
    (aVal, bVal) => {
      if (bVal === 0)
  ...
  sin: /* @__PURE__ */ __name((arg) => over(arg, Math.sin, "sin"), "sin"),
  cos: /* @__PURE__ */ __name((arg) => over(arg, Math.cos, "cos"), "cos"),
  tan: /* @__PURE__ */ __name((arg) => over(arg, Math.tan, "tan"), "tan"),
  atan: /* @__PURE__ */ __name((arg) => over(arg, Math.atan, "atan"), "atan"),
  asin: /* @__PURE__ */ __name((arg) => {
  ...
  atan2: /* @__PURE__ */ __name((y, x) => {
    const yVal = extractNumber(y, "atan2");
    const xVal = extractNumber(x, "atan2");
    return new NumberSymbol(Math.atan2(yVal, xVal));
  }, "atan2"),
  // Hyperbolic functions
  sinh: /* @__PURE__ */ __name((arg) => over(arg, Math.sinh, "sinh"), "sinh"),
  cosh: /* @__PURE__ */ __name((arg) => over(arg, Math.cosh, "cosh"), "cosh"),
  tanh: /* @__PURE__ */ __name((arg) => over(arg, Math.tanh, "tanh"), "tanh"),
  asinh: /* @__PURE__ */ __name((arg) => over(arg, Math.asinh, "asinh"), "asinh"),
  acosh: /* @__PURE__ */ __name((arg) => {
  ...
  // Constants
  pi: /* @__PURE__ */ __name(() => new NumberSymbol(Math.PI), "pi")
};
...
var _FunctionsManager = class _FunctionsManager extends BaseManager {
  ...
  setupBuiltinFunctions() {
    ...
    this.registerFunction(
      ...
      (strSymbol, baseSymbol) => {
        ...
        return new NumberSymbol(parsed2);
      }
    );
  ...
  registerDynamicFunction(spec) {
    ...
    const impl = /* @__PURE__ */ __name((...args2) => {
      try {
...
__name(_FunctionsManager, "FunctionsManager");
...
var specName2 = /* @__PURE__ */ __name((spec) => spec.name.toLowerCase(), "specName");
...
${spec}`
  ...
  validateListItems(value2, itemsSpec, visitedUrls, basePath, depth) {
    const results = [];
    if (!(value2 instanceof ListSymbol)) {
      return results;
    }
  ...
  getValueMap(value2) {
    ...
    if (value2 instanceof TokenSymbol && value2.value instanceof Map) {
      return value2.value;
    }
    return null;
  }
...
var specName3 = /* @__PURE__ */ __name((spec) => spec.name.toLowerCase(), "specName");
...
var _UnitManager = class _UnitManager extends BaseManager {
  ...
  registerConversions(uri, spec) {
    spec.conversions?.forEach((conversion) => {
      ...
      const targetUri = conversion.target === "$self" ? uri : conversion.target;
      const { ast } = parseExpression(conversion.script.script);
      const fn2 = /* @__PURE__ */ __name((unit) => {
...
__name(_UnitManager, "UnitManager");
...
export { ASTNodes, ColorManager, ColorSpecificationSchema, Config, ConstantsSpecificationSchema, DEFAULT_LANGUAGE_OPTIONS, FunctionSpecificationSchema, FunctionsManager, Interpreter, Lexer, Parser, TokenResolver, UnitManager, UnitSpecificationSchema, ValidationSeverity, buildTokens, builders_exports, collectReferenceNodes, errorClasses, errorHelpers, evaluateExpression, fetchAndRegisterSchemas, fetchTokenScriptSchema, filterAST, flattenChildrenMap, flattenChildrenObject, getAffectedTokens, getBrokenReferences, getModifiedDependants, getRenamedReferences, parseExpression, processTokenSets, processTokens, renameReferences, walkAST };
//# sourceMappingURL=chunk-IU4UODCJ.js.map
//# sourceMappingURL=chunk-IU4UODCJ.js.map