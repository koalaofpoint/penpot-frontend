...
${spec}`
  ...
  validate(tokenType, value, visitedUrls = /* @__PURE__ */ new Set(), basePath = [], depth = 0) {
    console.log(`[TokenManager.validate] tokenType="${tokenType}" depth=${depth} path=${JSON.stringify(basePath)}`, {
      valueType: value?.constructor?.name,
      valueStr: value?.toString?.()?.substring(0, 100)
    });
  ...
  validateListItems(value, itemsSpec, visitedUrls, basePath, depth) {
    const results = [];
    console.log("[validateListItems] Starting list validation", {
      isListSymbol: value instanceof ListSymbol,
      itemsSpecType: itemsSpec.type,
      hasProperties: !!itemsSpec.properties
    });
    if (!(value instanceof ListSymbol)) {
      console.log("[validateListItems] NOT a ListSymbol, returning empty");
      return results;
    }
    const items = value.value;
    console.log("[validateListItems] List has", items.length, "items");
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const itemPath = [...basePath, i];
      console.log(`[validateListItems] Processing item ${i}:`, {
        itemType: item?.constructor?.name,
        itemStr: item?.toString?.()?.substring(0, 100)
      });
      if (itemsSpec.type === "object" && itemsSpec.properties) {
        const itemMap = this.getValueMap(item);
        console.log(`[validateListItems] Item ${i} map:`, itemMap ? `Map(${itemMap.size}) keys=[${Array.from(itemMap.keys()).join(",")}]` : "null");
        if (itemMap) {
          const propResults = this.validateObjectProperties(
            itemMap,
          ...
          console.log(`[validateListItems] Item ${i} propResults:`, JSON.stringify(propResults));
          results.push(...propResults);
        }
      }
  ...
  validateObjectProperties(valueMap, properties, visitedUrls, basePath, depth) {
    const results = [];
    console.log("[validateObjectProperties] Starting validation for path:", basePath, "properties:", Object.keys(properties));
    for (const [propName, propSpec] of Object.entries(properties)) {
      ...
      if (!propValue) {
        console.log(`[validateObjectProperties] Property "${propName}" not found in valueMap, skipping`);
        continue;
      }
      const referencedSpec = this.getSpec(typedPropSpec.url);
      if (!referencedSpec) {
        console.log(`[validateObjectProperties] Spec not found for URL "${typedPropSpec.url}", skipping`);
        continue;
      }
      console.log(`[validateObjectProperties] Validating "${propName}":`, {
        url: typedPropSpec.url,
        specName: referencedSpec.name,
        inputType: propValue?.constructor?.name,
        inputValue: propValue?.toString?.() ?? String(propValue)
      });
      const newVisited = new Set(visitedUrls);
      ...
      console.log(`[validateObjectProperties] Result for "${propName}":`, JSON.stringify(nestedResults));
      for (const result of nestedResults) {
        if (!result.valid) {
          results.push(result);
        }
    ...
    console.log("[validateObjectProperties] Final results:", JSON.stringify(results));
    return results;
  }
  /**
...
//# sourceMappingURL=chunk-HFLLWRRV.js.map
//# sourceMappingURL=chunk-HFLLWRRV.js.map