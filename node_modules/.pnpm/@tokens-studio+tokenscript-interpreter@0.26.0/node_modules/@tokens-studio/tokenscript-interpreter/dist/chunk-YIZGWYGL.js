...
var mathFunctions = {
  min: (...args) => overMany(args, (nums) => Math.min(...nums), "min"),
  max: (...args) => overMany(args, (nums) => Math.max(...nums), "max"),
  mod: (a, b) => {
    const aVal = extractNumber(a, "mod");
    const bVal = extractNumber(b, "mod");
    if (bVal === 0)
      throw new InterpreterError("FN_DIVISION_BY_ZERO" /* DIVISION_BY_ZERO */, {
        data: { functionName: "mod" }
      });
    const result = (aVal % bVal + bVal) % bVal;
  ...
  pow: (base, exp) => {
    const baseVal = extractNumber(base, "pow");
    const expVal = extractNumber(exp, "pow");
    return new NumberSymbol(baseVal ** expVal);
  },
  ...
  remainder: (a, b) => {
    const aVal = extractNumber(a, "remainder");
    const bVal = extractNumber(b, "remainder");
    if (bVal === 0) {
      throw new InterpreterError("FN_DIVISION_BY_ZERO" /* DIVISION_BY_ZERO */, {
...
var FunctionsManager = class _FunctionsManager extends BaseManager {
  ...
  setupBuiltinFunctions() {
    ...
    this.registerFunction("sum", createSumFunction(() => this.parentConfig?.unitManager));
...
//# sourceMappingURL=chunk-YIZGWYGL.js.map
//# sourceMappingURL=chunk-YIZGWYGL.js.map