...
function GridCellAreaLabel(props: GridCellAreaLabelProps) {
  const { origin, width, text } = props;
...
function GridCell(props: GridCellProps) {
  const { shape, cell, layoutData } = props;
  ...
  const transformedOrigin = gpt.transform(
    cellOrigin,
...
export function GridLayoutViewer(props: GridLayoutViewerProps) {
  const { shape, objects } = props;

  const boundsMap = useMemo(() => {
    return d.lazyMap(Object.keys(objects), (id) =>
      gsh.shapeToPoints(objects[id])
    );
  }, [objects]);

  const children = useMemo(() => {
    return cfh
      .getImmediateChildren(objects, shape.id)
      .filter((child: any) => !child.hidden)
      .map((child: any) => {
        const childPoints = gpo.parentCoordsBounds(
          child.points,
          shape.points
        );
  ...
  const layoutData = useMemo(() => {
    return gsg.calcLayoutData(shape, shape.points, children, boundsMap, objects);
  }, [shape, children, boundsMap, objects]);
